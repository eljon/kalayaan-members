...
async function captureReturned(opts = {}) {
  ...
  try {
    ...
    // LCR bounces through the identity provider on the way in, so the URL
    // right after goto is often mid-redirect. Let it settle before judging
    // the session — checking immediately reports a false "session expired".
    const settleBy = Date.now() + 20000;
    while (Date.now() < settleBy && onIdentityProvider(page.url())) {
      await page.waitForTimeout(1000);
    }
