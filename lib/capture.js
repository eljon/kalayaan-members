...
const MONTH_ACTION_ID =
  process.env.LCR_MONTH_ACTION || "70c2f1f5d9f72c21ed5e21b32ccae9543fa54cfac9";
...
async function postMonth(ctx, unitNumber, month, stateTree, onProgress, actionId) {
  ...
  const headers = {
    ...
    "next-action": actionId || MONTH_ACTION_ID,
  };
      ...
      async function healActionId() {
        ...
        for (const id of await harvestActionIds(page, ROUTE)) {
          ...
          if (await validateActionId(ctx, anchor.unitNumber, anchorMonth, id, stateTree)) {
            actionId = id;
            log(`self-healed the month-switch action id (now ${id})`);
            warnings.push("The LCR month-switch id had changed; recovered it automatically for this pull. Set LCR_MONTH_ACTION or update MONTH_ACTION_ID to keep it.");
            return true;
          }
        }
