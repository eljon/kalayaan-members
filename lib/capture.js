...
// A custom report rendered as a plain table. There is no rich nested
// payload worth chasing here, so we read the rendered table straight from
// the DOM with the saved session — no flight parsing, no console dance.
const RETURNED_URL =
...
// Map each column to a field by matching the header text. Position-
// independent, so a column reorder on LCR's side does not silently
// misalign the data.
const RM_HEADER_MAP = [
  ...
  [/\bage\b/i, "age"],
  [/temple recommend status/i, "trStatus"],
  [/temple recommend expiration/i, "trExpiration"],
  [/calling/i, "callings"],
];

async function autoScroll(page) {
  // Force any lazily-rendered rows in before scraping.
...
async function captureReturned(opts = {}) {
  ...
  try {
    ...
    onProgress("Reading the table");
    const scraped = await page.evaluate((mapSrc) => {
      const map = mapSrc.map(([src, flags, field]) => [new RegExp(src, flags), field]);
      const tables = [...document.querySelectorAll("table")];
      const table = tables.find((t) => /preferred name/i.test(t.textContent)) || tables[0];
      if (!table) return { headers: [], records: [] };

      const headEls = table.querySelectorAll("thead th");
      const headers = (headEls.length
        ? [...headEls]
        : [...table.querySelectorAll("tr")][0].children
      ).map((h) => h.innerText.trim().replace(/\s+/g, " "));

      const fieldByCol = headers.map((h) => {
        const hit = map.find(([re]) => re.test(h));
      ...
      for (const tr of rows) {
        ...
        const rec = {};
        cells.forEach((v, i) => { const f = fieldByCol[i]; if (f) rec[f] = v; });
        if (rec.name) records.push(rec);
      }
      return { headers, records };
    }, RM_HEADER_MAP.map(([re, field]) => [re.source, re.flags, field]));

    log(`returned report headers: ${scraped.headers.join(" | ")}`);
    log(`returned report rows scraped: ${scraped.records.length}`);
