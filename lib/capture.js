...
// internals with NO stability guarantee. When LCR redeploys and rotates the
// id, the pull self-heals (harvests a fresh one live) and REMEMBERS it in
// lcr-action.json, so the recovery happens once, not every pull. Resolution
// order: LCR_MONTH_ACTION env var → remembered id → this baked-in default.
const DEFAULT_MONTH_ACTION_ID = "70c2f1f5d9f72c21ed5e21b32ccae9543fa54cfac9";
const ACTION_ID_PATH = path.join(__dirname, "..", "lcr-action.json");
function readSavedActionId() {
  try {
    const j = JSON.parse(fs.readFileSync(ACTION_ID_PATH, "utf8"));
    return j && typeof j.monthAction === "string" ? j.monthAction : null;
  } catch (_) { return null; }
}
function saveActionId(id) {
  try { fs.writeFileSync(ACTION_ID_PATH, JSON.stringify({ monthAction: id }, null, 2)); } catch (_) {}
}
function resolveMonthActionId() {
  return process.env.LCR_MONTH_ACTION || readSavedActionId() || DEFAULT_MONTH_ACTION_ID;
}
...
// Forget the saved LCR sign-in (delete the stored cookies/storage state), so
// the next pull requires a fresh login. Safe to call when none exists.
const clearSession = () => { try { fs.rmSync(SESSION_PATH, { force: true }); } catch (_) {} };
 ...
 */
async function postMonth(ctx, unitNumber, month, stateTree, onProgress, actionId) {
  onProgress(`Reading ${month}`);
  const headers = {
    accept: "text/x-component",
    "content-type": "text/plain;charset=UTF-8",
    "next-action": actionId || resolveMonthActionId(),
  };
  ...
  if (!res.ok()) {
    const s = res.status();
    // A 404/400 on the month-switch action almost always means the pinned
    // Next.js action id went stale on an LCR deploy — the one brittle value
    // in this pull. Say so, and how to fix it, right in the warning.
    const hint = (s === 404 || s === 400)
      ? " — the LCR month-switch action id is out of date (it changes whenever LCR redeploys). Capture a fresh next-action id from a signed-in browser and set LCR_MONTH_ACTION; see DISCOVERY.md."
      : "";
    throw new Error(`Month ${month} returned HTTP ${s}${hint}`);
  }
  return res.text();
}

/**
 * Self-heal a rotated action id. When LCR redeploys and the pinned
 * MONTH_ACTION_ID stops working, the page itself still holds the current id
 * — it fires it whenever a control changes. So drive the page's own
 * controls, sniff the `next-action` header off the resulting POST(s), and
 * return every candidate id observed. Non-destructive: we only read request
 * headers, so selecting the "wrong" value or month does no harm, and the
 * anchor month is already captured by the time this runs.
 */
async function harvestActionIds(page, route) {
  const ids = new Set();
  const onReq = (req) => {
    try {
      if (req.method() !== "POST") return;
      const id = req.headers()["next-action"];
      if (id && req.url().includes(route)) ids.add(id);
    } catch (_) { /* header read raced */ }
  };
  page.on("request", onReq);
  try {
    // Nudge every native <select> to a different option; the month/period
    // control is among them, and its change fires the server action.
    await page.evaluate(() => {
    ...
    await page.waitForTimeout(2500); // let any action POSTs fly
  } catch (_) { /* page navigated or closed — fall through */ }
  page.off("request", onReq);
  return [...ids];
}

/**
 * Does this action id actually work? Probe it against a month we know has
 * data (the anchor month) and confirm the response is a real flight. Lets
 * us trust a harvested id only after it proves itself, and never adopt a
 * wrong one.
 */
async function validateActionId(ctx, unitNumber, month, actionId, stateTree) {
  try {
    const headers = {
      accept: "text/x-component",
      "content-type": "text/plain;charset=UTF-8",
    ...
    return body.includes("weekOptions") && body.includes("members");
  } catch (_) {
    return false;
  }
}
...
async function capture(opts = {}) {
  ...
  try {
    ...
    if (anchorYear && anchorMonth) {
      ...
      // The action id LCR expects can rotate on a deploy. Start with the
      // pinned one; if a month POST 404s, harvest a fresh id off the live
      // page once and carry on with it. `actionId` is what we're using now.
      let actionId = resolveMonthActionId();
      let healTried = false;
      async function healActionId() {
        if (healTried) return false;
        healTried = true;
        log("month-switch action id rejected; harvesting a fresh one from the page");
        for (const id of await harvestActionIds(page, ROUTE)) {
          if (id === actionId) continue;
          if (await validateActionId(ctx, anchor.unitNumber, anchorMonth, id, stateTree)) {
            actionId = id;
            saveActionId(id); // remember it, so the next pull uses it directly
            // Success needs no action from the user, so it stays a quiet
            // terminal note — not an on-screen warning. Only a month that
            // actually failed to pull is surfaced in the UI.
            log(`self-healed the month-switch action id (now ${id}); saved to lcr-action.json`);
            return true;
          }
        }
        log("could not harvest a working action id; earlier months will be skipped");
        return false;
      }
      ...
      for (const { year, month } of targets) {
        ...
        try {
          let flight;
          try {
            flight = await postMonth(ctx, anchor.unitNumber, month, stateTree, onProgress, actionId);
          } catch (e) {
            // A stale action id shows up as a 404/400. Try to self-heal once,
            // then retry this month with the recovered id.
            if (/HTTP 40[04]/.test(e.message) && (await healActionId())) {
              flight = await postMonth(ctx, anchor.unitNumber, month, stateTree, onProgress, actionId);
            } else {
              throw e;
            }
