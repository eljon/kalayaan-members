...
async function captureReturned(opts = {}) {
  ...
  try {
    ...
    if (!best) {
      // Keep the payload for diagnosis rather than failing blind.
      const dump = path.join(__dirname, "..", "output", "returned-raw.txt");
      fs.mkdirSync(path.dirname(dump), { recursive: true });
      fs.writeFileSync(dump, bodies.map((b) => `=== ${b.url} (${b.size}) ===\n${b.body}`).join("\n\n"));
      log(`returned: no record array found; raw payload written to ${dump}`);
      throw new Error(
        "Could not find the records in the report payload. Raw response saved to output/returned-raw.txt for inspection."
      );
    }
