...
// (bytes C3 B1) surfaces as "Ã±". The attendance flight payload arrives this
// way; detect the tell-tale two-byte pattern and re-decode. A string that is
// already correct (a lone ñ is outside the trigger range) or not pure
// Latin-1 is returned untouched.
function fixMojibake(s) {
  ...
  const decoded = Buffer.from(codes).toString("utf8");
...
/**
 * Find the report object in a flight payload. Prefers the documented line
 * `1:`, but falls back to scanning every line for the one that parses as
 * JSON and carries the report shape. The month-switch POST is a server
 * action whose response may frame the data on a different line than the
 * initial GET, so relying on line 1 alone is not safe there.
 */
function extractReportData(flight) {
  const lines = flight.split(/\n(?=[0-9a-f]+:)/);
  const parseLine = (l) => {
    const i = l.indexOf(":");
    if (i < 0) return null;
    try {
      return JSON.parse(l.slice(i + 1));
    } catch (_) {
      return null;
    }
  };
  const looksRight = (d) => d && Array.isArray(d.weekOptions) && Array.isArray(d.members);

  const one = lines.find((l) => l.startsWith("1:"));
  if (one) {
    const d = parseLine(one);
  ...
  throw new Error("No report data found in flight payload");
}
...
function parse(data) {
  ...
  const walk = (orgs, depth = 0) => {
    for (const o of orgs) {
      const name = fixMojibake(o.name);
      orgNames[o.uuid] = name;
      orgList.push({ uuid: o.uuid, name, depth });
  ...
  const rows = members.map((m) => {
    ...
    return {
      name: fixMojibake(m.name),
      nameSort: fixMojibake(m.nameSort || m.name),
      uuid: m.uuid,
      gender: m.gender,
      orgIds: m.orgIds || [],
...
module.exports = { extractLine1, extractReportData, parse, mergeMonths, toCSV };
