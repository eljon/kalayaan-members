(() => {
  ...
  function renderCrTable() {
    ...
    const total = (members.records || []).length;
    $("unit-line").textContent = `Custom report · ${rows.length} of ${total} members · ${cols.length} columns`;
  ...
  function crRefreshSaved() {
    ...
    Object.keys(saved).sort().forEach((name) => { const o = el("option", null, name); o.value = name; sel.appendChild(o); });
  }
  function crSave() {
  ...
  function crApplySaved(name) {
    ...
    const r = saved[name];
    if (!r) return;
    // migrate the old flat {match, filters} shape into a single group
    const groups = r.groups
      ? r.groups.map((g) => ({ match: g.match || "all", filters: (g.filters || []).map((f) => ({ ...f })) }))
      : [{ match: r.match || "all", filters: (r.filters || []).map((f) => ({ ...f })) }];
    crState = { groupsMatch: r.groupsMatch || "all", groups: groups.length ? groups : [{ match: "all", filters: [] }], columns: (r.columns || []).slice() };
    $("cr-match").value = crState.groupsMatch;
    renderCrGroups(); renderCrColumns(); renderCrTable();
  }
  function crDelete() {
