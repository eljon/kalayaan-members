(() => {
  ...
  // LCR names a couple of columns after the report field they came from
  // rather than what they hold. Retitle those for display; the column KEY is
  // untouched, so saved reports and lists keep working.
  const COLUMN_LABEL_FIXES = {
    "progress baptism date": "Baptism Date",
    "birth date with example": "Birthday",
  };
  function tidyMemberColumns(m) {
    for (const c of (m && m.columns) || []) {
      const fix = COLUMN_LABEL_FIXES[String(c.label || "").trim().toLowerCase()];
      if (fix) c.label = fix;
    }
    return m;
  }
