(() => {
  ...
  function rmVisible() {
    ...
    const part = $("rm-participation").value;
    ...
    if (part) rows = rows.filter((r) => r._partStatus === part);
  ...
  function renderReturned() {
    ...
    // Live count in the eyebrow: shown as "X of Y" when filters are active,
    // just "Y people" when everything's showing.
    const total = (returned.records || []).length;
    $("unit-line").textContent =
      rows.length === total
        ? `Returned missionaries · ${total} people`
        : `Returned missionaries · ${rows.length} of ${total} people`;
  ...
  function renderMembers() {
    ...
    const rows = memVisible();
    $("mem-empty").hidden = rows.length > 0;

    const total = (members.records || []).length;
    $("unit-line").textContent =
      rows.length === total
        ? `All members · ${total} people · ${cols.length} fields`
        : `All members · ${rows.length} of ${total} people · ${cols.length} fields`;
  ...
  // ------------------------------------------------------------ custom reports
  // A client-side query builder over the all-members report: AND/OR filters,
  // a column chooser, live results, save/load, CSV. All in the browser — the
  // members data is already here.
  const CR_OPS = [
    { v: "contains", label: "contains", needsValue: true },
    { v: "is", label: "is", needsValue: true },
    { v: "is_not", label: "is not", needsValue: true },
    ...
    { v: "not_empty", label: "is not empty", needsValue: false },
  ];
  const CR_LS_KEY = "kalayaan:customReports";
  let crState = { match: "all", filters: [], columns: [] };
  let crReady = false;

  function crEval(cellRaw, op, val) {
    ...
    switch (op) {
      ...
      case "gt": { const a = parseFloat(cell), b = parseFloat(val); return !isNaN(a) && !isNaN(b) && a > b; }
      case "lt": { const a = parseFloat(cell), b = parseFloat(val); return !isNaN(a) && !isNaN(b) && a < b; }
      case "empty": return c === "";
  ...
  function crVisible() {
    ...
    const active = crState.filters.filter((f) => f.field && f.op);
    if (!active.length) return recs.slice();
    return recs.filter((r) => {
      const hits = active.map((f) => crEval(r[f.field], f.op, f.value));
      return crState.match === "any" ? hits.some(Boolean) : hits.every(Boolean);
    });
  }
  ...
  function renderCrFilters() {
    ...
    crState.filters.forEach((f, idx) => {
      ...
      fsel.onchange = () => { f.field = fsel.value; renderCrFilters(); renderCrTable(); };

      const osel = el("select", "cr-f-op");
      for (const op of CR_OPS) {
        const o = el("option", null, op.label); o.value = op.v;
        if (op.v === f.op) o.selected = true;
        osel.appendChild(o);
      }
      osel.onchange = () => { f.op = osel.value; renderCrFilters(); renderCrTable(); };

      const needsValue = (CR_OPS.find((o) => o.v === f.op) || {}).needsValue;
      const val = el("input", "cr-f-val");
      val.type = "text"; val.value = f.value || ""; val.placeholder = "value";
      if (!needsValue) val.style.visibility = "hidden";
      let vt;
      val.oninput = () => { f.value = val.value; clearTimeout(vt); vt = setTimeout(renderCrTable, 120); };

      const rm = el("button", "cr-f-remove"); rm.type = "button"; rm.textContent = "\u00d7"; rm.title = "Remove filter";
      rm.onclick = () => { crState.filters.splice(idx, 1); renderCrFilters(); renderCrTable(); };

      row.append(fsel, osel, val, rm);
      box.appendChild(row);
    });
    if (!crState.filters.length) {
      box.appendChild(el("p", "cr-hint", "No filters — showing everyone. Add a filter to narrow the report."));
    }
  }
  ...
  function renderCrTable() {
    ...
    const total = (members.records || []).length;
    $("unit-line").textContent =
      `Custom report · ${rows.length} of ${total} members · ${cols.length} columns`;
  ...
  function crSave() {
    const name = (prompt("Name this report:") || "").trim();
    if (!name) return;
    const saved = crLoadSaved();
    saved[name] = { match: crState.match, filters: crState.filters, columns: crState.columns };
    localStorage.setItem(CR_LS_KEY, JSON.stringify(saved));
    crRefreshSaved();
    $("cr-saved").value = name;
  }
  function crApplySaved(name) {
    const saved = crLoadSaved();
    const r = saved[name];
    if (!r) return;
    crState = {
    ...
    $("cr-match").value = crState.match;
    renderCrFilters(); renderCrColumns(); renderCrTable();
  }
  function crDelete() {
    const name = $("cr-saved").value;
    if (!name) return;
  ...
  $("cr-match").addEventListener("change", () => { crState.match = $("cr-match").value; renderCrTable(); });
  $("cr-add-filter").addEventListener("click", () => {
    const first = (members && members.columns[0]) ? members.columns[0].key : "";
    crState.filters.push({ field: first, op: "contains", value: "" });
    renderCrFilters(); renderCrTable();
  });
  $("cr-columns-btn").addEventListener("click", () => { $("cr-columns").hidden = !$("cr-columns").hidden; });
  $("cr-save").addEventListener("click", crSave);
  $("cr-delete").addEventListener("click", crDelete);
  $("cr-saved").addEventListener("change", () => crApplySaved($("cr-saved").value));
  ...
  $("rm-participation").addEventListener("change", renderReturned);
  let mt;
