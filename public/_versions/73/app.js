(() => {
  ...
  // One legend line, with an optional comparison delta ("prev" = the count
  // in the comparison period). Shared shape used across the stat widgets.
  function statLine({ cls, label, n, pct, prev, title, sub, onClick, active }) {
    ...
    if (prev != null) {
      const d = n - prev;
      li.appendChild(el("span", "stat-delta delta-" + (d > 0 ? "up" : d < 0 ? "down" : "flat"), (d > 0 ? "+" : "") + d));
    }
  ...
  function renderAttendanceStats(rows) {
    ...
    legend.appendChild(statLine({ cls: "yes", label: "Seen", n: seen, pct: pctOf(seen), prev: cmp && cmp.seen }));
    // Attendees broken down by the weeks-column colour tiers, grouped under Seen.
    const tiers = el("div", "stat-tiers");
    tiers.appendChild(statLine({ cls: "green", label: "Green (>50%)", n: s.green, pct: pctOf(s.green), prev: cmp && cmp.green, title: "Attended more than half the weeks", sub: true }));
    tiers.appendChild(statLine({ cls: "yellow", label: "Yellow (≤50%)", n: s.yellow, pct: pctOf(s.yellow), prev: cmp && cmp.yellow, title: "Attended half the weeks or fewer", sub: true }));
    tiers.appendChild(statLine({ cls: "orange", label: "Orange (≤25%)", n: s.orange, pct: pctOf(s.orange), prev: cmp && cmp.orange, title: "Attended a quarter of the weeks or fewer", sub: true }));
    legend.appendChild(tiers);

    legend.appendChild(statLine({ cls: "no", label: "Not seen", n: notSeen, pct: pctOf(notSeen), prev: cmp && cmp.notSeen }));
    legend.appendChild(statLine({ label: "Every week", n: s.everyWeek, prev: cmp && cmp.everyWeek, title: `Attended every week of the ${cmp ? "report" : ""} period` }));
  ...
  function renderReportStats(host, opts) {
    ...
    legend.appendChild(statLine({
      ...
      prev: compare && compare.yes, active: scope === "attending",
      onClick: () => onScope("attending"),
    }));
    // Attendee tier breakdown (green/yellow/orange), grouped under Attending.
    if (tiers) {
      const g = el("div", "stat-tiers");
      g.appendChild(statLine({ cls: "green", label: "Green (>50%)", n: tiers.green || 0, pct: pctOf(tiers.green || 0), prev: ct && (ct.green || 0), title: "Attended more than half the weeks", sub: true }));
      g.appendChild(statLine({ cls: "yellow", label: "Yellow (≤50%)", n: tiers.yellow || 0, pct: pctOf(tiers.yellow || 0), prev: ct && (ct.yellow || 0), title: "Attended half the weeks or fewer", sub: true }));
      g.appendChild(statLine({ cls: "orange", label: "Orange (≤25%)", n: tiers.orange || 0, pct: pctOf(tiers.orange || 0), prev: ct && (ct.orange || 0), title: "Attended a quarter of the weeks or fewer", sub: true }));
      legend.appendChild(g);
    }
    legend.appendChild(statLine({
      cls: "no", label: "Not attending", n: no, pct: pctOf(no),
      prev: compare && compare.no, active: scope === "not",
      onClick: () => onScope("not"),
    }));
    fig.appendChild(legend);
    body.appendChild(fig);
  ...
  function renderCrTable() {
    ...
    if (cmpOn) {
      ...
      compareStats = { yes: cc.yes, no: cc.no, tiers: tierCountsOf(cmpInfos) };
    }
