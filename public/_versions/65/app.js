(() => {
  ...
  function serializeRoll() {
    ...
    const rows = [...document.querySelectorAll("#roll-body .roll-row")].map((row) => {
      ...
      const score = (row.querySelector(".r-score") || {}).textContent || "";
      return { cells: [{ v: name.trim() }, ...(withClass ? [{ v: cls.trim() }] : []), ...weekCells, { v: score.trim(), align: "right" }] };
    });
