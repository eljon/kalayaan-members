(() => {
  ...
  const PREF_LS = {
    ...
    summaryHidden: "kalayaan:summaryHidden",
  };
  let _srvPrefs = null;
  ...
  const summaryHiddenMap = () => _loadPref("summaryHidden", {}) || {};
  function setPresetSummaryHidden(id, hidden) {
    const m = summaryHiddenMap();
    if (hidden) m["al:" + id] = true; else delete m["al:" + id];
    _savePref("summaryHidden", m);
  }
  const SUMMARY_SCOPES = {
    ...
    actions: {
      ...
      get: () => {
        ...
        if (!e) return false;
        return e.kind === "custom" ? !!e.list.state.summaryHidden : !!summaryHiddenMap()["al:" + e.id];
      },
      set: (v) => {
        const e = alActiveEntry();
        if (!e) return;
        if (e.kind === "custom") { e.list.state.summaryHidden = v; alSaveLists(); }
        else setPresetSummaryHidden(e.id, v);
      },
    },
  };
  ...
  function renderAlTable() {
    ...
    const baseCols = wantCols.map((k) => byKey.get(k)).filter(Boolean);
    ...
    for (const c of displayCols) {
      const sorted = alSort.key === c.key;
      const th = el("th", "rm-th" + (c.att ? " att-th" : "") + (sorted ? " sorted " + (alSort.dir > 0 ? "asc" : "desc") : ""), c.label);
      th.setAttribute("role", "button");
      th.onclick = () => {
        if (alSort.key === c.key) alSort.dir *= -1; else alSort = { key: c.key, dir: 1 };
        saveSort("actions", alSort);
        renderAlTable();
      };
      htr.appendChild(th);
    }
