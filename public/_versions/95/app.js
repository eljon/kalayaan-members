(() => {
  ...
  const qiAttendRatio = (pool, ctx) => ({
    actual: pool.filter((r) => qiAttended(r, ctx)).length,
    potential: pool.length,
  });
  ...
  const QI_ROWS = [
    ...
    { n: 1, label: "Individuals who are sealed to a spouse in the temple",
      ...
      calc: (c) => qiNeed("sealed", "marriage") && {
        actual: c.recs.filter((r) => qiYes(qiVal(r, "sealed")) || /seal/i.test(qiVal(r, "sealed"))).length,
        potential: c.recs.filter((r) => /married/i.test(qiVal(r, "marriage"))).length,
      } },
    { n: 2, label: "Endowed members with a current temple recommend",
      ...
      calc: (c) => qiNeed("endowed") && (() => {
        ...
        return { actual: pool.filter((r) => qiRecommendCurrent(r, c.end)).length, potential: pool.length };
      })() },
    { n: 3, label: "Males ages 18–25 serving or who have served a full-time mission",
      ...
      calc: (c) => qiNeed("rm", "gender") && (() => {
        ...
        return { actual: pool.filter((r) => qiYes(qiVal(r, "rm"))).length, potential: pool.length };
      })() },
    { n: 4, label: "Adult males holding Melchizedek Priesthood",
      ...
      calc: (c) => qiNeed("priesthood", "gender") && (() => {
        ...
        return { actual: pool.filter(qiIsMP).length, potential: pool.length };
      })() },
    ...
    { n: 6, label: "Young men with priesthood office appropriate for their age",
      ...
      calc: (c) => qiNeed("office", "gender") && (() => {
        ...
        return {
          actual: pool.filter((r) => qiOfficeRank(r) >= qiExpectedRank(qiAgeOn(r, c.end))).length,
          potential: pool.length,
        };
      })() },
    ...
    { n: 8, label: "Converts baptized and confirmed in the last 12 months", count: true,
      ...
      calc: (c) => { const v = qiConverts(c.recs, c.yearAgo, c.end); return v && { actual: v.length }; } },
    ...
    { g: "Members / Families" },
    { n: 10, label: "Total members", count: true, calc: (c) => ({ actual: c.recs.length }) },
    { n: 11, label: "Total families", count: true,
      why: "The member export carries no household field.",
      calc: (c) => qiNeed("household") && {
        actual: new Set(c.recs.map((r) => nameKey(qiVal(r, "household"))).filter(Boolean)).size,
      } },
    ...
    { n: 21, label: "Children ages 0 through 2 years (as of 1 January)", count: true,
      ...
      calc: (c) => callingCols().birthKey
        ? { actual: c.recs.filter((r) => qiInBand(r, c.jan1, 0, 2)).length }
        : false },
    ...
    { n: 24, label: "Youth and adult converts assigned a Church responsibility or calling",
      ...
      calc: (c) => qiNeed("callings") && (() => {
        ...
        return { actual: pool.filter((r) => qiVal(r, "callings")).length, potential: pool.length };
      })() },
    { n: 25, label: "Youth and adult male converts ordained to appropriate priesthood office",
      ...
      calc: (c) => qiNeed("office", "gender") && (() => {
        ...
        return { actual: pool.filter((r) => qiOfficeRank(r) >= qiExpectedRank(qiAgeOn(r, c.end))).length, potential: pool.length };
      })() },
  ...
  function renderQuarterly() {
    ...
    for (const r of rows) {
      ...
      const tr = el("tr", r.value ? null : "qi-row-na");
      ...
      if (!r.value) {
        ...
        const { actual, potential } = r.value;
