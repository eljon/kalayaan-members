(() => {
  ...
  function attStatsFrom(rows) {
    // Count within the selected report range, not the whole pulled span.
  ...
  function renderAttendanceStats(rows) {
    ...
    const item = (cls, label, n, pct, title, sub) => {
      const li = el("li", "stat-item" + (cls ? "" : " stat-note") + (sub ? " stat-sub" : ""));
    ...
    legend.appendChild(item("yes", "Seen", seen, pctOf(seen)));
    // Break the attendees down by the weeks-column colour tiers.
    legend.appendChild(item("green", "Green (>50%)", s.green, pctOf(s.green), "Attended more than half the weeks", true));
    legend.appendChild(item("yellow", "Yellow (≤50%)", s.yellow, pctOf(s.yellow), "Attended half the weeks or fewer", true));
    legend.appendChild(item("orange", "Orange (≤25%)", s.orange, pctOf(s.orange), "Attended a quarter of the weeks or fewer", true));
    legend.appendChild(item("no", "Not seen", notSeen, pctOf(notSeen)));
  ...
  function renderReportStats(host, opts) {
    const { scope, yes, no, shown, noun, onScope, tiers } = opts;
    if (!host) return;
    host.textContent = "";
    ...
    // Attendee tier breakdown (green/yellow/orange), subordinate to Attending.
    if (tiers) {
      const sub = (cls, label, n, title) => {
        const li = el("li", "stat-item stat-sub");
        li.appendChild(el("i", "dot dot-" + cls));
        li.appendChild(el("span", "stat-label", label));
        li.appendChild(el("b", "stat-count", String(n)));
        li.appendChild(el("span", "stat-pct", total ? Math.round((n / total) * 100) + "%" : "—"));
      ...
      legend.appendChild(sub("green", "Green (>50%)", tiers.green || 0, "Attended more than half the weeks"));
      legend.appendChild(sub("yellow", "Yellow (≤50%)", tiers.yellow || 0, "Attended half the weeks or fewer"));
      legend.appendChild(sub("orange", "Orange (≤25%)", tiers.orange || 0, "Attended a quarter of the weeks or fewer"));
    }
