(() => {
  ...
  function crParticipationYes(rec, op, value) {
    ...
    const attended = rg ? attendanceInfoRange(rec[nameCol], rg).total > 0 : false;
  ...
  const AL_PRESETS = [
    {
      ...
      available: () => !!data,
      match: (rec, nameCol) => attendanceInfoRange(rec[nameCol], alFullRange()).total === 0,
    },
    {
      id: "lapsed4",
      ...
      match: (rec, nameCol) => {
        if (attendanceInfoRange(rec[nameCol], alFullRange()).total === 0) return false; // that's "Never attended"
        return attendanceInfoRange(rec[nameCol], alRecentRange(4)).total === 0;
      },
    },
    {
      id: "nocalling",
  ...
  const qiAttended = (rec, ctx) => {
    const a = attendanceInfoRange(rec[ctx.nameCol], ctx.range);
    return a.weeks === 0 || a.total > 0;
  };
