(() => {
  ...
  // ------------------------------------------------------------ line graph
  // Hand-rolled inline SVG, no chart library. Plots members present per
  // week; left axis is the count, right axis the same as a percentage of
  // the roll, and each point is labelled with both.
  function renderGraph() {
    ...
    for (let t = 0; t <= ticks; t++) {
      ...
      grid += `<text x="${mL + pW + 7}" y="${(+gy + 3).toFixed(1)}" class="g-axis g-axis-r">${Math.round((val / roll) * 100)}%</text>`;
    }

    const pts = present.map((v, i) => `${x(i).toFixed(1)},${y(v).toFixed(1)}`);
    const area = `<polygon class="g-area" points="${mL},${(mT + pH).toFixed(1)} ${pts.join(" ")} ${mL + pW},${(mT + pH).toFixed(1)}"/>`;
    const line = `<polyline class="g-line" points="${pts.join(" ")}"/>`;

    let dots = "";
    present.forEach((v, i) => {
      const cx = x(i).toFixed(1);
      const cy = y(v);
      dots += `<circle cx="${cx}" cy="${cy.toFixed(1)}" r="2.6" class="g-dot"/>`;
      dots += `<text x="${cx}" y="${(cy - 9).toFixed(1)}" class="g-val">${v}</text>`;
      dots += `<text x="${cx}" y="${(cy - 20).toFixed(1)}" class="g-pct">${pct(v)}%</text>`;
    });
    ...
    box.innerHTML =
      ...
      `aria-label="Members present per week across ${n} weeks">` +
      `${grid}${area}${line}${dots}${xlab}</svg>`;
  }
