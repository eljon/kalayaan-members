(() => {
  ...
  async function ensureReturned() {
    ...
    try {
      ...
      if (!res.ok) {
        showRmNotice(
          "No returned-missionary data pulled yet. Capture it with dev/console/7-custom-report.js, then it appears here."
        );
        return;
      }
  ...
  function updateRmContext() {
    ...
    if (returned.sample) {
      ...
      $("rm-notice").textContent =
        "Sample data. The real report has not been pulled from LCR yet — see dev/console/7-custom-report.js.";
    } else {
