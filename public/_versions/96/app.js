(() => {
  ...
  function qiOpenIndicator(row, ctx) {
    ...
    qiOpenRow = { row, ctx };
    const v = row.value;
    const pool = v.potential || v.actual;
    ...
    const pct = v.potential && v.potential.length
      ? Math.round((v.actual.length / v.potential.length) * 100) + "%" : null;
    $("qi-modal-meta").textContent =
      `${ctx.quarter.label} · as of ${qiHumanDate(ctx.end)} · ` +
      (v.potential
        ? `${v.actual.length} of ${v.potential.length} ${unit}${pct ? " · " + pct : ""}`
        : `${v.actual.length} ${unit}`);

    const body = $("qi-modal-body");
    body.textContent = "";
    if (!pool.length) {
      body.appendChild(el("p", "modal-msg", "Nobody falls into this group for the selected quarter."));
    } else {
      // Those still to act on first — the reason for opening the row.
      const rows = pool.slice().sort((a, b) => {
        const ina = inActual.has(nameKey(a[ctx.nameCol])) ? 1 : 0;
        const inb = inActual.has(nameKey(b[ctx.nameCol])) ? 1 : 0;
        if (ina !== inb) return ina - inb;
        return String(a[ctx.nameCol] || "").localeCompare(String(b[ctx.nameCol] || ""));
      });
      ...
      if (v.potential) htr.appendChild(el("th", "rm-th", "Counts"));
      htr.appendChild(el("th", "rm-th", "Attendance"));
      thead.appendChild(htr);
      const tbody = el("tbody");
      rows.forEach((r, i) => {
        const nm = String(r[ctx.nameCol] || "");
        const yes = inActual.has(nameKey(nm));
        const tr = el("tr", v.potential && !yes ? "qi-gap-row" : null);
        tr.appendChild(numTd(i, "mem-td"));
        const td = el("td", "mem-td");
  ...
  function qiDownloadCsv() {
    ...
    const head = ["Name"].concat(v.potential ? ["Counts"] : []).concat(["Attendance"]);
    const lines = [head.join(",")];
    for (const r of pool) {
      const nm = String(r[ctx.nameCol] || "");
      const cells = [nm];
      if (v.potential) cells.push(inActual.has(nameKey(nm)) ? "Yes" : "No");
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `${row.n} ${row.label} ${ctx.quarter.label}`.replace(/[^\w .-]+/g, "").trim() + ".csv";
    document.body.appendChild(a); a.click(); a.remove();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
  }
