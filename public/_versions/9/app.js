(() => {
  ...
  // ------------------------------------------------------------ line graph
  // Hand-rolled inline SVG, no chart library. Two lines on independent
  // axes: members present per week (count, left axis) and attendance as a
  // percentage of the roll (right axis, fixed 0-100%). The percentage line
  // is the same trend on a fixed scale, so it sits low — a visual reminder
  // that most of the roll is not here.
  function renderGraph() {
    const box = $("graph");
    if (!box) return;
    const wt = data.weekTotals || [];
    const roll = (data.stats && data.stats.members) || 0;
    const n = wt.length;
    if (!n || !roll) { box.innerHTML = ""; return; }

    const W = 1000, H = 260;
    const mL = 40, mR = 44, mT = 30, mB = 46;
    ...
    const yCount = (v) => mT + pH * (1 - v / countMax);   // left scale
    const yPct = (p) => mT + pH * (1 - p / 100);          // right scale, 0-100%
    const pct = (v) => Math.round((v / roll) * 100);

    // Horizontal gridlines follow the left (count) axis.
    let grid = "";
    const ticks = 4;
    for (let t = 0; t <= ticks; t++) {
      const val = (countMax * t) / ticks;
      const gy = yCount(val).toFixed(1);
    ...
    // Right axis ticks are the percentage scale, at their own positions.
    for (const p of [0, 25, 50, 75, 100]) {
      const gy = (yPct(p) + 3).toFixed(1);
      grid += `<text x="${mL + pW + 7}" y="${gy}" class="g-axis g-axis-r">${p}%</text>`;
    }

    const countPts = present.map((v, i) => `${x(i).toFixed(1)},${yCount(v).toFixed(1)}`);
    const ratePts = present.map((v, i) => `${x(i).toFixed(1)},${yPct(pct(v)).toFixed(1)}`);
    ...
    const rateLine = `<polyline class="g-line g-line-rate" points="${ratePts.join(" ")}"/>`;

    let dots = "";
    present.forEach((v, i) => {
      const cx = x(i).toFixed(1);
      const cyc = yCount(v);
      const cyr = yPct(pct(v));
      dots += `<circle cx="${cx}" cy="${cyc.toFixed(1)}" r="2.6" class="g-dot g-dot-count"/>`;
      dots += `<text x="${cx}" y="${(cyc - 8).toFixed(1)}" class="g-val">${v}</text>`;
      dots += `<circle cx="${cx}" cy="${cyr.toFixed(1)}" r="2.4" class="g-dot g-dot-rate"/>`;
      dots += `<text x="${cx}" y="${(cyr + 15).toFixed(1)}" class="g-pctval">${pct(v)}%</text>`;
    });
    ...
    for (const g of monthGroups()) {
      ...
      xlab += `<text x="${cx}" y="${H - 16}" class="g-month">${g.label}</text>`;
    }

    const legend =
      `<div class="g-legend">` +
      `<span class="g-key g-key-count">Present (number)</span>` +
      `<span class="g-key g-key-rate">Attending (% of roll)</span>` +
      `</div>`;

    box.innerHTML =
      legend +
      `<svg viewBox="0 0 ${W} ${H}" class="graph-svg" role="img" ` +
      `aria-label="Members present and attendance rate per week across ${n} weeks">` +
      `${grid}${area}${countLine}${rateLine}${dots}${xlab}</svg>`;
  }
