(() => {
  ...
  // ---- Attendance date range + comparison -----------------------------
  ...
  function reportRange() {
    const n = weekCount();
    if (!attRange) return { start: 0, end: Math.max(0, n - 1) };
    let s = clampWeek(attRange.start), e = clampWeek(attRange.end);
    if (s > e) [s, e] = [e, s];
    return { start: s, end: e };
  }
  function compareRange() {
    let s = clampWeek(attCompare.start), e = clampWeek(attCompare.end);
    if (s > e) [s, e] = [e, s];
    return { start: s, end: e };
  }
