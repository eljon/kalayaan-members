(() => {
  ...
  function renderAttendanceStats(rows) {
    ...
    tiers.appendChild(statLine({ cls: "green", label: "Green (>50%)", n: s.green, pct: pctOf(s.green), title: "Attended more than half the weeks", sub: true }));
    tiers.appendChild(statLine({ cls: "yellow", label: "Yellow (≤50%)", n: s.yellow, pct: pctOf(s.yellow), title: "Attended half the weeks or fewer", sub: true }));
  ...
  // Roll a per-week series up into month or quarter buckets. Each bucket's
  // value is the AVERAGE of its weeks, counting only weeks that actually had
  // attendance (value > 0) — weeks with no meeting or no entry are excluded so
  // a skipped Sunday doesn't drag the average down. `weekIdx` maps each series
  // position to a global weekOptions index, used to date each week. Returns the
  // bucket values plus x-axis ticks ({ top, group }). Weekly granularity passes
  // through unchanged (ticks null; the caller uses calendar labels instead).
  function aggregateWeekly(values, weekIdx, gran) {
    if (gran !== "month" && gran !== "quarter") return { values: values.slice(), ticks: null };
    const wo = data.weekOptions || [];
    const groups = [];
    const byKey = new Map();
    values.forEach((v, i) => {
      ...
      let g = byKey.get(key);
      if (!g) { g = { year: y, month: m, vals: [] }; byKey.set(key, g); groups.push(g); }
      g.vals.push(v);
    });
  ...
  function renderGraph() {
    ...
    const agg = aggregateWeekly(seriesFor(rr), rrIdx, gran);
    ...
    const countLabel = gran === "week"
      ? noun + " each week"
      : noun + " · avg / " + (gran === "month" ? "month" : "quarter");
    const perPhrase = gran === "week" ? "per week"
      : gran === "month" ? "averaged per month" : "averaged per quarter";

    // Segmented weekly / monthly / quarterly control, plus the visitors toggle.
    ...
    if (attCompare.on) {
      ...
      const cagg = aggregateWeekly(seriesFor(cr), idxOf(cr), gran);
    ...
    box.innerHTML =
      `<div class="graph-controls">${segGroup}${toggle}</div>` +
      attendanceGraphHtml(
        present,
        `Members${withVis ? " and visitors" : ""} present ${perPhrase}${trendPhrase}, across ${present.length} ${unitPlural}`,
        countLabel,
        opts
      );
    const cb = $("g-visitors");
  ...
  // The custom-report graph: same weekly / monthly / quarterly control and the
  // same aggregation (monthly/quarterly = average weekly attendance, empty
  // weeks excluded) as the roll graph, but scoped to this report's members and
  // date range. Re-renders just #cr-graph so toggling granularity doesn't
  ...
  function renderCrGraph() {
    ...
    const agg = aggregateWeekly(counts.slice(rr.start, rr.end + 1), rrIdx, gran);
    const present = agg.values;

    const periodNoun = gran === "month" ? "Month" : gran === "quarter" ? "Quarter" : "Week";
    const countLabel = gran === "week" ? "Present each week"
    ...
    if (cmpOn) {
      const cagg = aggregateWeekly(counts.slice(cr.start, cr.end + 1), idxOf(cr.start, cr.end), gran);
    ...
    const trendPhrase = cmpOn ? ", report vs comparison period"
      : gran === "week" ? ", with a polynomial trend" : "";
    g.innerHTML =
      `<div class="graph-controls">${segGroup}</div>` +
      attendanceGraphHtml(
        present,
        `Report members present ${perPhrase}${trendPhrase}`,
        countLabel,
        gopts
      );
  ...
  function renderReportStats(host, opts) {
    ...
    if (tiers) {
      ...
      g.appendChild(statLine({ cls: "green", label: "Green (>50%)", n: tiers.green || 0, pct: pctOf(tiers.green || 0), title: "Attended more than half the weeks", sub: true }));
      g.appendChild(statLine({ cls: "yellow", label: "Yellow (≤50%)", n: tiers.yellow || 0, pct: pctOf(tiers.yellow || 0), title: "Attended half the weeks or fewer", sub: true }));
