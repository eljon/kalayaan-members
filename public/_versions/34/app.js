(() => {
  ...
  function attendanceInfo(name) {
    ...
    return { status: attended > 0 ? "yes" : "no", total: attended, weeks: elig, pct, text: `${attended}/${elig} ${pct}%` };
  }
  ...
  function renderCrTable() {
    ...
    const base = sortRows(crVisible(), crSort, nameColKey); // query-matched + sorted
  ...
  function crDownloadCsv() {
    ...
    const nameColKey = (members.columns.find((c) => /name/.test(c.key)) || {}).key;
    const displayCols = withAttendanceCol(cols, nameColKey);
    const rows = sortRows(crVisible(), crSort, nameColKey).filter((r) => inScope(partOf(r[nameColKey]), crScope));
