(() => {
  ...
  // Attendance for one person, as "x/y z%" (weeks attended / weeks in the
  // span, and the percentage). "—" when they aren't on the attendance roll.
  function attendanceInfo(name) {
    const weeks = (data && data.weekOptions) ? data.weekOptions.length : 0;
    const row = attendanceFor(name);
    const total = row ? row.total : 0;
    const status = !row ? "unknown" : total > 0 ? "yes" : "no";
    const pct = weeks && row ? Math.round((total / weeks) * 100) : 0;
    return { status, total, weeks, pct, text: row ? `${total}/${weeks} ${pct}%` : "—" };
  }
  ...
  function rmBase() {
    ...
    let rows = (returned.records || []).map((r) => {
      ...
      return Object.assign({}, r, {
        ...
        participation:
          p.status === "unknown" ? "—" :
          `${p.total}/${p.weeks} ${p.weeks ? Math.round((p.total / p.weeks) * 100) : 0}%`,
      });
    });
    if (q) rows = rows.filter((r) => (r.name || "").toLowerCase().includes(q));
  ...
  function renderCrTable() {
    ...
    const thead = el("thead");
    const htr = el("tr");
    for (const c of displayCols) htr.appendChild(el("th", "rm-th" + (c.att ? " att-th" : ""), c.label));
    thead.appendChild(htr);
  ...
  function crDownloadCsv() {
    ...
    const nameColKey = (members.columns.find((c) => /name/.test(c.key)) || {}).key;
    const displayCols = withAttendanceCol(cols, nameColKey);
    const rows = crVisible().filter((r) => inScope(partOf(r[nameColKey]), crScope));
