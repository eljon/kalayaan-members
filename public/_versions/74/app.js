(() => {
  ...
  function renderAttendanceStats(rows) {
    ...
    if (cmp) fig.appendChild(compareCaption(rangeLabel(reportRange()), rangeLabel(compareRange())));
    const legend = el("ul", "stat-legend");

    legend.appendChild(statLine({ cls: "yes", label: "Seen", n: seen, pct: pctOf(seen), cmpN: cmp && cmp.seen, cmpPct: cmp && pctOf(cmp.seen) }));
    // Attendees broken down by the weeks-column colour tiers, grouped under Seen.
    const tiers = el("div", "stat-tiers");
    tiers.appendChild(statLine({ cls: "green", label: "Green (>50%)", n: s.green, pct: pctOf(s.green), cmpN: cmp && cmp.green, cmpPct: cmp && pctOf(cmp.green), title: "Attended more than half the weeks", sub: true }));
    tiers.appendChild(statLine({ cls: "yellow", label: "Yellow (≤50%)", n: s.yellow, pct: pctOf(s.yellow), cmpN: cmp && cmp.yellow, cmpPct: cmp && pctOf(cmp.yellow), title: "Attended half the weeks or fewer", sub: true }));
    tiers.appendChild(statLine({ cls: "orange", label: "Orange (≤25%)", n: s.orange, pct: pctOf(s.orange), cmpN: cmp && cmp.orange, cmpPct: cmp && pctOf(cmp.orange), title: "Attended a quarter of the weeks or fewer", sub: true }));
    legend.appendChild(tiers);

    legend.appendChild(statLine({ cls: "no", label: "Not seen", n: notSeen, pct: pctOf(notSeen), cmpN: cmp && cmp.notSeen, cmpPct: cmp && pctOf(cmp.notSeen) }));
    legend.appendChild(statLine({ label: "Every week", n: s.everyWeek, cmpN: cmp && cmp.everyWeek, title: "Attended every week of the period" }));
  ...
  function attendanceGraphHtml(present, ariaLabel, countLabel, opts) {
    ...
    // X-axis labels. Calendar dates + month names normally; plain week
    // numbers when comparing two different calendar periods.
    ...
    if (comparing) {
      ...
      xlab = `<text x="${((mL + mL + pW) / 2).toFixed(1)}" y="${H - 12}" class="g-month">Week of the period</text>`;
    } else {
      present.forEach((v, i) => {
  ...
  function renderGraph() {
    ...
    const present = seriesFor(rr);
    const weekIndices = [];
    for (let i = rr.start; i <= rr.end; i++) weekIndices.push(i);

    const toggle =
      `<label class="g-toggle"><input type="checkbox" id="g-visitors"${withVis ? " checked" : ""}> ` +
      `Include visitors</label>`;

    const opts = { weekIndices };
    if (attCompare.on) {
      const cr = compareRange();
      opts.compare = { present: seriesFor(cr), label: "Compare · " + rangeLabel(cr) };
    ...
    box.innerHTML =
      ...
      attendanceGraphHtml(
        present,
        `Members${withVis ? " and visitors" : ""} present per week${attCompare.on ? ", report period vs comparison period" : " with a polynomial trend"}, across ${present.length} weeks`,
  ...
  function renderHead(showClass) {
    ...
    if (attCompare.on) {
      head.appendChild(el("div", "h-score h-compare", "vs prior"));
  ...
  function renderBody() {
    ...
    for (const [i, r] of rows.entries()) {
      ...
      // Comparison range + the change (count and rate) between the two.
      if (comparing) {
        const cAtt = attendedIn(r, cr);
        row.appendChild(scoreCell(cAtt, cWeeks));
        const dCount = rAtt - cAtt;
        ...
        const sign = (v) => (v > 0 ? "+" : "") + v;
        row.appendChild(el("div", "r-delta delta-" + dir, `${sign(dCount)} · ${sign(dPct)}%`));
      }
  ...
  function renderReportStats(host, opts) {
    ...
    if (compare) fig.appendChild(compareCaption(compare.labelA, compare.labelB));
    const legend = el("ul", "stat-legend");

    legend.appendChild(statLine({
      cls: "yes", label: "Attending", n: yes, pct: pctOf(yes),
      cmpN: compare && compare.yes, cmpPct: compare && pctOf(compare.yes),
    ...
    if (tiers) {
      ...
      g.appendChild(statLine({ cls: "green", label: "Green (>50%)", n: tiers.green || 0, pct: pctOf(tiers.green || 0), cmpN: ct && (ct.green || 0), cmpPct: ct && pctOf(ct.green || 0), title: "Attended more than half the weeks", sub: true }));
      g.appendChild(statLine({ cls: "yellow", label: "Yellow (≤50%)", n: tiers.yellow || 0, pct: pctOf(tiers.yellow || 0), cmpN: ct && (ct.yellow || 0), cmpPct: ct && pctOf(ct.yellow || 0), title: "Attended half the weeks or fewer", sub: true }));
      g.appendChild(statLine({ cls: "orange", label: "Orange (≤25%)", n: tiers.orange || 0, pct: pctOf(tiers.orange || 0), cmpN: ct && (ct.orange || 0), cmpPct: ct && pctOf(ct.orange || 0), title: "Attended a quarter of the weeks or fewer", sub: true }));
      legend.appendChild(g);
    }
    legend.appendChild(statLine({
      cls: "no", label: "Not attending", n: no, pct: pctOf(no),
      cmpN: compare && compare.no, cmpPct: compare && pctOf(compare.no),
  ...
  function renderCrTable() {
    ...
    // Weekly attendance graph for just this report's members, scoped to the
    // report range, and overlaid with the comparison range when active.
    const g = $("cr-graph");
    if (g) {
      const counts = weeklyCountsFor(base, nameColKey);
      const present = counts.slice(rr.start, rr.end + 1);
      const weekIndices = [];
      for (let i = rr.start; i <= rr.end; i++) weekIndices.push(i);
      const gopts = { weekIndices };
      if (cmpOn) {
        gopts.compare = { present: counts.slice(cr.start, cr.end + 1), label: "Compare · " + rangeLabel(cr) };
        gopts.labelA = "Report · " + rangeLabel(rr);
      }
      g.innerHTML = attendanceGraphHtml(
        present,
        `Report members present per week${cmpOn ? ", report vs comparison period" : ", with a polynomial trend"}`,
        "Present each week",
        gopts
      );
    }
    ...
    // Label the comparison column with its range, then compose the columns.
    ...
    for (const [i, r] of rows.entries()) {
      ...
      for (const c of displayCols) {
        if (c.att) {
          ...
          const td = el("td", "mem-td att-td");
          if (c.delta) {
            const dp = crDeltaPct(nm);
            const rp = attendanceInfoRange(nm, rr), cp = attendanceInfoRange(nm, cr);
            const dCount = (rp.total || 0) - (cp.total || 0);
            const dir = dp > 0 ? "up" : dp < 0 ? "down" : "flat";
            const sign = (v) => (v > 0 ? "+" : "") + v;
            td.appendChild(el("span", "r-delta delta-" + dir, `${sign(dCount)} · ${sign(dp)}%`));
          } else {
  ...
  function crDownloadCsv() {
    ...
    const cell = (c, r) => {
      ...
      if (c.delta) {
        const dp = crDeltaPct(nm);
        const dCount = (attendanceInfoRange(nm, rr).total || 0) - (attendanceInfoRange(nm, cr).total || 0);
        const sign = (v) => (v > 0 ? "+" : "") + v;
        return `${sign(dCount)} · ${sign(dp)}%`;
      }
