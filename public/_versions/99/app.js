(() => {
  ...
  // Server-side preferences with localStorage fallback. The server stores
  // everything in output/prefs.json; localStorage is a fast cache so the
  // first paint (theme, font) doesn't flash.
  const PREF_LS = {
    sort: "kalayaan:sort",
    settings: "kalayaan:settings",
    customReports: "kalayaan:customReports",
    actionLists: "kalayaan:actionLists",
    actionDone: "kalayaan:actionDone",
    presetPrefs: "kalayaan:presetPrefs",
  };
  let _srvPrefs = null;

  async function initPrefs() {
    try { _srvPrefs = await (await fetch("/api/prefs")).json(); } catch (_) { _srvPrefs = {}; }
    let dirty = false;
    for (const [key, lsKey] of Object.entries(PREF_LS)) {
      if (_srvPrefs[key] === undefined) {
        try {
          const v = JSON.parse(localStorage.getItem(lsKey));
          if (v != null) { _srvPrefs[key] = v; dirty = true; }
        } catch (_) {}
      }
    }
    if (dirty) {
      fetch("/api/prefs", {
        ...
        body: JSON.stringify(_srvPrefs),
      }).catch(() => {});
    }
  }

  function _loadPref(key, fallback) {
    if (_srvPrefs && _srvPrefs[key] !== undefined) return _srvPrefs[key];
    const lsKey = PREF_LS[key];
    if (lsKey) {
      try { const v = JSON.parse(localStorage.getItem(lsKey)); if (v != null) return v; } catch (_) {}
    }
    return fallback;
  }

  function _savePref(key, value) {
    if (!_srvPrefs) _srvPrefs = {};
    _srvPrefs[key] = value;
    const lsKey = PREF_LS[key];
    if (lsKey) try { localStorage.setItem(lsKey, JSON.stringify(value)); } catch (_) {}
    fetch("/api/prefs", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  function loadSorts() { return _loadPref("sort", {}); }
  ...
  function loadSettings() {
    const s = _loadPref("settings", {});
    return {
      theme: s.theme === "dark" ? "dark" : "light",
      font: FONTS.some((f) => f.name === s.font) ? s.font : DEFAULT_FONT,
    };
  }
  function saveSettings(s) { _savePref("settings", s); }
  ...
  // The weeks of `range` a person could actually have attended: everything
  // after their baptism Sunday. THE one place that rule lives — every count of
  // "attending" is derived from it, so none of them can drift apart. When it
  // returns nothing there was no Sunday to miss, and the person counts as
  // attending rather than being marked absent for weeks that never applied.
  function eligibleWeeks(baptNum, range) {
    const weekNums = attIndex().weekNums;
    const s = Math.max(0, range.start), e = Math.min(weekNums.length - 1, range.end);
    const out = [];
    for (let i = s; i <= e; i++) {
      if (baptNum != null) { const wd = weekNums[i]; if (wd != null && wd <= baptNum) continue; }
      out.push(i);
    }
    return out;
  }
  ...
  function attStatsFrom(rows, range) {
    ...
    let seen = 0, everyWeek = 0, green = 0, yellow = 0, orange = 0, blank = 0;
    for (const r of rows) {
      const a = attendedIn(r, rg);
      // No eligible Sunday in the range — nothing to have missed, so seen.
      if (!eligibleWeeks(baptismDateOf(r.nameSort || r.name), rg).length) {
        seen += 1; blank += 1;
        continue;
      }
      if (a > 0) seen += 1;
    ...
    return { seen, notSeen: rows.length - seen, total: rows.length, everyWeek, weeks, green, yellow, orange, blank };
  }
  ...
  function renderAttendanceStats(rows) {
    ...
    if (cmp) {
      ...
      fig.appendChild(compareTable([
        ...
        ...(s.blank || cmp.blank ? [{ label: "Not yet counted", sub: true, rN: s.blank || 0, rPct: pctOf(s.blank || 0), cN: cmp.blank || 0, cPct: pctOf(cmp.blank || 0) }] : []),
    ...
    if (s.blank) {
      tiers.appendChild(statLine({ label: "Not yet counted", n: s.blank, pct: pctOf(s.blank), title: "No eligible Sunday in the period — baptized after the last week pulled", sub: true }));
    }
  ...
  async function boot() {
    ...
    await initPrefs();
    // Re-apply settings from server (handles cross-machine sync)
    const ss = loadSettings();
    applyTheme(ss.theme);
    applyFont(ss.font);
    const seg = $("theme-seg");
    if (seg) seg.querySelectorAll(".seg-btn").forEach((b) => b.classList.toggle("is-active", b.dataset.theme === ss.theme));
    const fsel = $("font-select");
    if (fsel) fsel.value = ss.font;
    // Re-apply sort/graph prefs
    const sv = loadSorts();
    if (sv.attendance && [...$("sort").options].some((o) => o.value === sv.attendance)) $("sort").value = sv.attendance;
    attGraphGran = GRANS.indexOf(sv.attGran) >= 0 ? sv.attGran : "week";
    attGraphTotal = sv.attTotal === true;
    attGraphVisitors = sv.attVisitors === true;
    crGraphGran = GRANS.indexOf(sv.crGran) >= 0 ? sv.crGran : "week";
    crGraphTotal = sv.crTotal === true;
    // Re-apply action list prefs
    alLists = alLoadLists();
    alDone = alLoadDone();
    alActiveId = sv.actionsActive || null;
    alSort = sv.actions || { key: null, dir: 1 };
    ...
    if (s.hasData) {
      ...
      renderOrgs();
      renderAll();
      // Baptism dates live in the member report; the roll needs them to know
      // who had no eligible Sunday. Fetch it quietly and repaint once.
      if (!members) loadMembersData().then((m) => { if (m && activeTab === "attendance") renderAll(); });
    ...
    try {
      ...
      if (!expired) {
        try {
          ...
          if (res.ok) members = tidyMemberColumns(await res.json());
      ...
      // Action-list badges reflect live data, so refresh them after any pull.
      if (members) refreshActionBadges();
  ...
  function setTab(tab) {
    ...
    $("view-actions").hidden = tab !== "actions";
    $("view-quarterly").hidden = tab !== "quarterly";
    // The roll's Seen figure needs baptism dates to know who had no eligible
    // Sunday, and those live in the member report. Load it in the background
    // and repaint once, so the count does not depend on which tabs have been
    // opened.
    if (tab === "attendance" && !members) {
      loadMembersData().then((m) => { if (m && activeTab === "attendance") renderAll(); });
    }
    if (tab === "returned") {
      ...
      ensureAttendance().finally(() => ensureActions());
    } else if (tab === "quarterly") {
      ensureAttendance().finally(() => ensureQuarterly());
    }
  }
  ...
  function computeAttendanceInfo(name) {
    ...
    if (!row) return { status: "yes", tier: "unknown", total: 0, weeks: 0, pct: 0, text: "—" };
    const cells = row.cells || [];
    const idx = eligibleWeeks(baptismDateOf(name), { start: 0, end: attIndex().weekNums.length - 1 });
    const elig = idx.length;
    let attended = 0;
    for (const i of idx) if (cells[i]) attended += 1;
    // "Attending" means attended at least once in the period, so the roll's
    // seen/attending totals and this agree.
    const status = attended > 0 ? "yes" : "no";
    // Baptized on/after the last pulled Sunday: no eligible Sundays yet, so
    // there is nothing to have missed. A blank counts as attending.
    if (elig === 0) return { status: "yes", tier: "unknown", total: 0, weeks: 0, pct: 0, text: "—" };
  ...
  function attendanceInfoRange(name, range) {
    ...
    if (!row) return { status: "yes", tier: "unknown", total: 0, weeks: 0, pct: 0, text: "—" };
    const cells = row.cells || [];
    const idx = eligibleWeeks(baptismDateOf(name), range);
    const elig = idx.length;
    let attended = 0;
    for (const i of idx) if (cells[i]) attended += 1;
    // No eligible Sunday in the range — nothing to have missed.
    if (elig === 0) return { status: "yes", tier: "unknown", total: 0, weeks: 0, pct: 0, text: "—" };
    const pct = Math.round((attended / elig) * 100);
    return { status: attended > 0 ? "yes" : "no", tier: attendanceTier(pct), total: attended, weeks: elig, pct, text: `${attended}/${elig} ${pct}%` };
  }

  // The single test for "counts as attending", so the donut, the scope tabs,
  // the action lists, the participation filter and the quarterly indicators
  // cannot disagree about the same person. A blank — no eligible Sunday in
  // the period — counts, since there was nothing to miss.
  const countsAsAttending = (name, range) => attendanceInfoRange(name, range).status === "yes";
  ...
  // Break a set of attendance infos into the weeks-column colour tiers, so
  // green+yellow+orange+blank equals the "attending" total. `blank` is those
  // with no eligible Sunday to attend — they count as attending but have no
  // percentage to place them in a tier.
  function tierCountsOf(infos) {
    let green = 0, yellow = 0, orange = 0, blank = 0;
    for (const a of infos) {
      if (!a) continue;
      if (a.weeks === 0) { blank += 1; continue; }
      if (a.total > 0) {
        if (a.pct >= 50) green += 1;
    ...
    return { green, yellow, orange, blank };
  }
  ...
  // Which scope tab a person belongs to. "Not attending" means seen on no
  // eligible Sunday of the period; someone with no eligible Sunday at all
  // counts as attending rather than being held against weeks they could not
  // have attended.
  ...
  function renderReportStats(host, opts) {
    ...
    if (compare) {
      ...
      if (tiers && ct) {
        ...
        if (tiers.blank || ct.blank) {
          trows.push({ label: "Not yet counted", sub: true, rN: tiers.blank || 0, rPct: pctOf(tiers.blank || 0), cN: ct.blank || 0, cPct: pctOf(ct.blank || 0) });
        }
      }
    ...
    if (tiers) {
      ...
      if (tiers.blank) {
        g.appendChild(statLine({ label: "Not yet counted", n: tiers.blank, pct: pctOf(tiers.blank), title: "No eligible Sunday in the period — baptized after the last week pulled, or not on the roll", sub: true }));
      }
  ...
  // LCR names some columns after the report section they came from rather
  // than what they hold — "Progress Record Baptism Date", "Birth Date with
  // Example". Match on how the label ENDS so any section prefix is covered,
  // however LCR words it. The column KEY is untouched: keys are derived from
  // the original label, and changing them would break saved reports and lists.
  const COLUMN_LABEL_FIXES = [
    [/baptism\s*date$/i, "Baptism Date"],
    [/birth\s*date(\s+with\s+example)?$/i, "Birthday"],
  ];
  function tidyMemberColumns(m) {
    for (const c of (m && m.columns) || []) {
      const label = String(c.label || "").trim();
      const fix = COLUMN_LABEL_FIXES.find(([re]) => re.test(label));
      if (fix) c.label = fix[1];
    }
    return m;
  }
  ...
  function loadMembersData() {
    ...
    membersLoad = fetch("/api/members")
      ...
      .then((d) => { members = tidyMemberColumns(d); return members; })
      .catch(() => null)
  ...
  async function ensureMembers() {
    ...
    try {
      ...
      members = tidyMemberColumns(await res.json());
      renderMembers();
    } catch (_) {
  ...
  function openProfile(name) {
    ...
    loadMembersData().then((m) => {
      ...
      for (const c of filled.concat(empty)) {
        ...
        const dd = el("dd");
        dd.appendChild(nameOrText(v, name));
        cell.appendChild(dd);
        dl.appendChild(cell);
      }
      body.appendChild(dl);
  ...
  function closeProfile() {
    ...
    // The drill-down may still be open underneath, so leave the scroll lock.
    if ($("qi-modal").hidden) document.body.classList.remove("modal-open");
  }
  ...
  // A value is a person if it resolves to somebody in the all-members report.
  // Testing the value rather than the column name means a field LCR calls
  // "Head Of House", "Spouse Name" or anything else links without being listed
  // here, a class like "Gospel Doctrine" never does, and a name we hold no
  // profile for stays plain text instead of becoming a dead link.
  let _memNames = null, _memNamesFor = null;
  function memberNameKeys() {
    if (_memNamesFor === members) return _memNames;
    const set = new Set();
    const col = memNameKey();
    if (members && col) {
      for (const r of members.records || []) {
        const k = nameKey(r[col]);
        if (k) set.add(k);
      }
    }
    _memNames = set; _memNamesFor = members;
    return set;
  }
  function isMemberName(v) {
    const s = String(v == null ? "" : v).trim();
    if (s.length < 3 || !/[A-Za-z]/.test(s)) return false;
    return memberNameKeys().has(nameKey(s));
  }
  // A cell's contents: clickable when the value names a member, plain
  // otherwise. `except` suppresses a link back to the profile already open.
  function nameOrText(v, except) {
    const s = String(v == null ? "" : v).trim();
    if (!s) return document.createTextNode("—");
    if (isMemberName(s) && (!except || nameKey(s) !== nameKey(except))) return nameLink(s);
    return document.createTextNode(s);
  }
  // Same, as a table cell.
  function nameOrTextTd(v, cls) {
    const td = el("td", cls);
    td.appendChild(nameOrText(v));
    return td;
  }
  ...
  function renderMemRecon(nameCol) {
    ...
    const line = (items, label, nameOf) => {
      ...
      const div = el("div", "recon-line", `${items.length} ${label}: `);
      names.slice(0, 40).forEach((nm, i) => {
        if (i) div.appendChild(document.createTextNode("; "));
        div.appendChild(nameOrText(nm));
      });
      if (names.length > 40) div.appendChild(document.createTextNode(" …"));
      recon.appendChild(div);
    };
    ...
    console.warn("[Kalayaan] In all-members but not on the roll:", memOnly.map((r) => r[nameCol]));
    console.warn("[Kalayaan] On the roll but not in all-members:", rollOnly.map((r) => r.name));
  }
    ...
    for (const [i, r] of rows.entries()) {
      ...
      for (const c of displayCols) {
        ...
        if (c.key === nameColKey && String(v || "").trim()) {
          ...
          tr.appendChild(nameOrTextTd(v, "mem-td"));
        }
      }
      frag.appendChild(tr);
    }
  ...
  const CR_OPS = {
    ...
    // Participation reads the attendance roll rather than the member report,
    // so its operators are about showing up, and its value is a time window.
    participation: [
      { v: "att_once", label: "attended at least once" },
      { v: "att_never", label: "never attended" },
    ],
  };
  // Custom reports are stored in the server-side prefs under "customReports".

  // Synthetic fields a condition may filter on that aren't columns in the
  // member report; each is computed per member wherever a condition uses it.
  // The two age bands are keyed off the "age a member reaches this calendar
  // year" rule — the same turning-12 test used for callings. Participation
  // is different in kind: it reads the attendance roll, not the report.
  const CR_VFIELDS = [
    { key: "__primary",      label: "Primary age",   virtual: true },
    { key: "__youth",        label: "Youth age",     virtual: true },
    { key: "__participation", label: "Participation", virtual: true },
  ];
  ...
  // ------------------------------------------------------- participation
  // "Attended at least once (or never) in the past N weeks/months/quarters",
  // answered from the attendance roll. The value is stored the same way the
  // relative date filter stores its own — a plain string like "3 months".
  //
  // The window is anchored on the LAST week in the roll, not on today, so a
  // stale pull still asks a meaningful question instead of quietly matching
  // an empty range of weeks.
  const CR_PART_UNITS = [["week", "weeks"], ["month", "months"], ["quarter", "quarters"]];
  const CR_PART_RE = /^(\d+)\s*(week|month|quarter)s?$/i;
  const CR_PART_DEFAULT = "3 months";

  function crShiftMonths(num, delta) {
    const y = Math.floor(num / 10000), mo = Math.floor(num / 100) % 100, d = num % 100;
    const dt = new Date(y, mo - 1 + delta, 1);
    const lastDay = new Date(dt.getFullYear(), dt.getMonth() + 1, 0).getDate();
    dt.setDate(Math.min(d, lastDay)); // 31 Mar back one month is 28 Feb, not 3 Mar
    return dt.getFullYear() * 10000 + (dt.getMonth() + 1) * 100 + dt.getDate();
  }

  // Week-index range covered by a participation window, or null when the roll
  // has no weeks in it at all. An unset/unparseable window means the whole roll.
  function crParticipationRange(value) {
    const weekNums = attIndex().weekNums;
    const n = weekNums.length;
    if (!n) return null;
    const m = CR_PART_RE.exec(String(value == null ? "" : value).trim());
    if (!m) return { start: 0, end: n - 1 };
    const qty = Math.max(1, parseInt(m[1], 10));
    const unit = m[2].toLowerCase();
    // Weeks are counted in pulled Sundays, which is what the roll actually has.
    if (unit === "week") return { start: Math.max(0, n - qty), end: n - 1 };
    const last = weekNums[n - 1];
    if (last == null) return { start: 0, end: n - 1 };
    const cutoff = crShiftMonths(last, -(unit === "quarter" ? qty * 3 : qty));
    for (let i = 0; i < n; i++) {
      if (weekNums[i] != null && weekNums[i] > cutoff) return { start: i, end: n - 1 };
    }
    return null;
  }

  // Someone missing from the roll counts as not having attended — the same
  // reading the "Never attended" preset uses.
  function crParticipationYes(rec, op, value) {
    const nameCol = memNameKey();
    const rg = nameCol ? crParticipationRange(value) : null;
    const attended = rg ? countsAsAttending(rec[nameCol], rg) : false;
    return op === "att_never" ? !attended : attended;
  }
  ...
  function crGroupMatches(group, rec) {
    ...
    const hits = active.map((f) => {
      if (f.field === "__participation") return crParticipationYes(rec, f.op, f.value);
  ...
  // Records matching a filter state ({groupsMatch, groups}). No active filters
  // anywhere → everyone. Shared by Custom Reports and Action Lists.
  function crVisibleFrom(state) {
    const recs = (members && members.records) || [];
    const groupResults = (rec) => state.groups.map((g) => crGroupMatches(g, rec)).filter((x) => x !== null);
    return recs.filter((rec) => {
      const rs = groupResults(rec);
      if (!rs.length) return true;
      return state.groupsMatch === "any" ? rs.some(Boolean) : rs.every(Boolean);
    });
  }
  function crVisible() { return crVisibleFrom(crState); }
  ...
  function crInit() {
    ...
    crTypes.__participation = "participation";
    crState = { groupsMatch: "all", groups: [{ match: "all", filters: [] }], columns: crDefaultColumns(), description: "", summaryHidden: false, columnsCustomized: false };
    crReady = true;
  }
  ...
  function crFieldCombo(f, ctx) {
    ctx = ctx || crDefaultCtx();
    const cols = crFields();
  ...
  function crFilterRow(group, f, ctx) {
    ctx = ctx || crDefaultCtx();
    const row = el("div", "cr-filter");

    const fsel = crFieldCombo(f, ctx);
    ...
    osel.onchange = () => { f.op = osel.value; ctx.rebuild(); };

    row.append(fsel, osel, crValueInput(f, type, ctx));
  ...
  function crValueInput(f, type, ctx) {
    ctx = ctx || crDefaultCtx();
    ...
    if (type === "boolean") {
      ...
      sel.onchange = () => { f.value = sel.value; ctx.refresh(); };
      return sel;
    }
    if (type === "participation") return crParticipationValueInput(f, ctx);
    if (type === "date") return crDateValueInput(f, ctx);
    const inp = el("input", "cr-f-val");
    inp.type = type === "number" ? "number" : type === "date" ? "date" : "text";
    inp.value = f.value || "";
    inp.placeholder = "value";
    let vt;
    inp.oninput = () => { f.value = inp.value; clearTimeout(vt); vt = setTimeout(ctx.refresh, 120); };
    inp.onchange = () => { f.value = inp.value; ctx.refresh(); };
    return inp;
  }

  // Participation value control: "in the past [ 3 ] [ months ]".
  function crParticipationValueInput(f, ctx) {
    ctx = ctx || crDefaultCtx();
    // A brand-new condition arrives with an empty value; give it a window so
    // the row means something the moment it appears.
    if (!CR_PART_RE.test(String(f.value || "").trim())) f.value = CR_PART_DEFAULT;
    const m = CR_PART_RE.exec(f.value);
    const wrap = el("span", "cr-f-val cr-f-partval");

    const num = document.createElement("input");
    num.className = "cr-f-partnum";
    num.type = "number"; num.min = "1"; num.step = "1";
    num.value = m[1];

    const unitSel = el("select", "cr-f-partunit");
    for (const [v, label] of CR_PART_UNITS) {
      const o = el("option", null, label); o.value = v;
      if (v === m[2].toLowerCase()) o.selected = true;
      unitSel.appendChild(o);
    }

    // The row reads as a sentence, so the unit agrees with the number.
    const relabel = (n) => {
      [...unitSel.options].forEach((o, i) => { o.textContent = CR_PART_UNITS[i][n === 1 ? 0 : 1]; });
    };
    relabel(+m[1]);
    ...
    const commit = () => {
      ...
      num.value = String(n);
      relabel(n);
      f.value = `${n} ${unitSel.value}${n === 1 ? "" : "s"}`;
      ctx.refresh();
    };
    let vt;
    num.oninput = () => { clearTimeout(vt); vt = setTimeout(commit, 150); };
    num.onchange = commit;
    unitSel.onchange = commit;

    wrap.append(el("span", "cr-f-partlead", "in the past"), num, unitSel);
    return wrap;
  }
  ...
  function crDateValueInput(f, ctx) {
    ctx = ctx || crDefaultCtx();
  ...
  function crHumanFilter(f) {
    ...
    // Phrased to sit after "Members where …", like the other clauses.
    if (type === "participation") {
      const m = CR_PART_RE.exec(String(v || "").trim());
      const when = m ? `in the past ${m[1]} ${m[2].toLowerCase()}${+m[1] === 1 ? "" : "s"}` : "on record";
      return `attendance ${when} is ` + (f.op === "att_never" ? "none" : "at least once");
    }
    if (type === "number") {
  ...
  // The filter editor renders against a context so both Custom Reports and
  // Action Lists can share it: { state, container, rebuild, refresh }. Called
  // with no ctx it defaults to the Custom Reports state and DOM, so existing
  // callers are unchanged. rebuild() re-renders the editor + its table; the
  // lighter refresh() re-renders only the table (used while typing a value).
  function crDefaultCtx() {
    return {
      state: crState,
      container: $("cr-filters"),
      rebuild: () => { renderCrGroups(); renderCrTable(); },
      refresh: () => renderCrTable(),
    };
  }
  function renderCrGroups(ctx) {
    ctx = ctx || crDefaultCtx();
    ...
    state.groups.forEach((g, gi) => {
      ...
      g.filters.forEach((f) => card.appendChild(crFilterRow(g, f, ctx)));
      box.appendChild(card);
      if (gi < state.groups.length - 1) {
        box.appendChild(el("div", "cr-group-conn", state.groupsMatch === "all" ? "AND" : "OR"));
      }
    });
  }

  // ------------------------------------------------- columns, shared
  // Custom Reports and Action Lists show the same table over the same member
  // report, so choosing columns, rearranging them and sorting are one
  // implementation. Each surface supplies a context saying where its column
  // list and sort live; nothing below knows which tab it is drawing.
  function crColCtx() {
    return {
      box: () => $("cr-columns"),
      get: () => crState.columns,
      set: (cols) => { crState.columns = cols; crMarkColsCustomized(); },
      sort: () => crSort,
      setSort: (v) => { crSort = v; saveSort("custom", v); },
      redrawTable: () => renderCrTable(),
      redraw: () => { renderColumnPicker(crColCtx()); renderCrTable(); },
    };
  }
  // A preset list has no saved definition, so its chosen columns live in the
  // preset prefs; a custom list keeps them on the list itself.
  function alColumnsOf(entry) {
    if (!entry) return crDefaultColumns();
    if (entry.kind === "custom") {
      return entry.list.state.columnsCustomized ? entry.list.state.columns : crDefaultColumns();
    }
    const p = alPresetPrefs(entry.id);
    return p.columnsCustomized ? (p.columns || []) : crDefaultColumns();
  }
  function alColCtx() {
    const entry = alActiveEntry();
    return {
      box: () => $("al-columns"),
      get: () => alColumnsOf(entry),
      set: (cols) => {
        if (!entry) return;
        if (entry.kind === "custom") {
          entry.list.state.columns = cols;
          entry.list.state.columnsCustomized = true;
          alSaveLists();
        } else {
          setAlPresetPrefs(entry.id, { columns: cols, columnsCustomized: true });
        }
      },
      sort: () => alSort,
      setSort: (v) => { alSort = v; saveSort("actions", v); },
      redrawTable: () => renderAlTable(),
      redraw: () => { renderColumnPicker(alColCtx()); renderAlTable(); },
    };
  }

  function renderColumnPicker(ctx) {
    const box = ctx.box();
    if (!box) return;
    box.textContent = "";
    if (!members) return;
    ...
    all.onclick = () => { ctx.set(cols.map((c) => c.key)); ctx.redraw(); };
    const none = el("button", "cr-linkbtn", "Clear"); none.type = "button";
    none.onclick = () => { ctx.set([]); ctx.redraw(); };
    bar.append(all, none);
    box.appendChild(bar);
    const grid = el("div", "cr-col-grid");
    const chosen = ctx.get();
    for (const c of cols) {
      const lab = el("label", "cr-col");
      const cb = el("input"); cb.type = "checkbox"; cb.checked = chosen.includes(c.key);
      cb.onchange = () => {
        const cur = ctx.get().slice();
        if (cb.checked) { if (!cur.includes(c.key)) cur.push(c.key); }
        else { const i = cur.indexOf(c.key); if (i >= 0) cur.splice(i, 1); }
        ctx.set(cur);
        ctx.redrawTable();
      };
  ...
  function reorderColumns(fromKey, toKey, ctx) {
    if (!fromKey || fromKey === toKey) return;
    const cols = ctx.get().slice();
  ...
  let colDragKey = null;  // column being dragged in a table header
  let colDragged = false; // suppress the sort click that ends a drag
  // One header cell: click to sort, drag onto another to rearrange. The
  // synthetic attendance columns are pinned, so they only sort.
  function columnTh(c, ctx) {
    const st = ctx.sort();
    const sorted = st.key === c.key;
    const th = el("th", "rm-th" + (c.att ? " att-th" : " cr-th-move") +
      (sorted ? " sorted " + (st.dir > 0 ? "asc" : "desc") : ""), c.label);
    th.setAttribute("role", "button");
    th.onclick = () => {
      if (colDragged) return; // a drag just ended on this header; don't sort
      const cur = ctx.sort();
      ctx.setSort(cur.key === c.key ? { key: c.key, dir: cur.dir * -1 } : { key: c.key, dir: 1 });
      ctx.redrawTable();
    };
    if (!c.att) {
      th.draggable = true;
      th.addEventListener("dragstart", (e) => {
        colDragKey = c.key; colDragged = false;
  ...
  function renderCrTable() {
    ...
    const htr = el("tr");
    htr.appendChild(numTh());
    const crCols = crColCtx();
    for (const c of displayCols) {
      const th = columnTh(c, crCols);
      htr.appendChild(th);
    }
    thead.appendChild(htr);
  ...
  // ======================================================================
  // Action Lists — lists we work down to zero. Presets (attendance/roster
  // based, always current) plus user-built lists that reuse the exact Custom
  // Reports filter builder. Each list shows a live red count; rows drop off
  // automatically when a fresh pull no longer matches, and each row also has a
  // manual "Done" check-off (remembered per list in this browser).
  // ======================================================================
  // Action lists and check-offs are stored in server-side prefs.

  function alFullRange() { return resolveRange(null); }
  function alRecentRange(n) {
    const end = weekCount() - 1;
    return { start: Math.max(0, end - (n - 1)), end };
  }

  // Preset lists. `match(rec, nameCol)` decides membership; `available()` hides
  // a preset when the data it needs isn't present.
  const AL_PRESETS = [
    {
      id: "never",
      name: "Never attended",
      desc: "Members with no recorded attendance in any week — the first people to reach out to.",
      available: () => !!data,
      match: (rec, nameCol) => !countsAsAttending(rec[nameCol], alFullRange()),
    },
    {
      id: "lapsed4",
      name: "Not seen in 4+ weeks",
      desc: "Attended before, but not in the last 4 weeks — slipping into less activity.",
      available: () => !!data && weekCount() >= 4,
      match: (rec, nameCol) => {
        if (!countsAsAttending(rec[nameCol], alFullRange())) return false; // that's "Never attended"
        return !countsAsAttending(rec[nameCol], alRecentRange(4));
      },
    },
    {
      id: "nocalling",
      name: "No calling",
      desc: "Members old enough for a calling (turning 12 or older this year) who don't have one recorded.",
      available: () => !!(members && (members.columns || []).find(isCallingColumn)),
      match: (rec) => {
        const col = (members.columns || []).find(isCallingColumn);
        return !!col && callingEligible(rec) && !String(rec[col.key] || "").trim();
      },
    },
    {
      id: "norecommend",
      name: "No active temple recommend",
      desc: "Members old enough for a temple recommend whose status isn't active or current.",
      available: () => !!(members && (members.columns || []).find(isTempleRecommendColumn)),
      match: (rec) => {
        const col = (members.columns || []).find(isTempleRecommendColumn);
        if (!col || !callingEligible(rec)) return false;
        const v = String(rec[col.key] || "").trim();
        return !v || !/active|current|valid/i.test(v);
      },
    },
  ];

  function alLoadLists() { return _loadPref("actionLists", []) || []; }
  function alSaveLists() { _savePref("actionLists", alLists); }
  function alLoadDone() { return _loadPref("actionDone", {}) || {}; }
  function alSaveDone() { _savePref("actionDone", alDone); }

  let alLists = alLoadLists();
  let alDone = alLoadDone();
  let alActiveId = loadSorts().actionsActive || null;
  let alShowDone = false;
  let alEditing = false;
  let alSort = loadSorts().actions || { key: null, dir: 1 };
  let alWired = false;
  let alAwaitingMembers = false;

  function alDoneSet(id) { return new Set(alDone[id] || []); }
  function alSetDone(id, key, on) {
    const set = alDoneSet(id);
    if (on) set.add(key); else set.delete(key);
    alDone[id] = [...set];
    alSaveDone();
  }
  function alNewState() {
    return { groupsMatch: "all", groups: [{ match: "all", filters: [] }], columns: crDefaultColumns(), description: "", summaryHidden: false, columnsCustomized: false };
  }
  function alAllLists() {
    const presets = AL_PRESETS.filter((p) => !p.available || p.available())
      .map((p) => ({ id: p.id, name: p.name, kind: "preset", preset: p }));
    const customs = alLists.map((l) => ({ id: l.id, name: l.name, kind: "custom", list: l }));
    return presets.concat(customs);
  }
  function alFindList(id) { return alAllLists().find((x) => x.id === id) || null; }
  function alActiveEntry() {
    const all = alAllLists();
    let e = all.find((x) => x.id === alActiveId);
    if (!e && all.length) { e = all[0]; alActiveId = e.id; }
    return e || null;
  }

  function alMatchingRecords(entry) {
    if (!members || !entry) return [];
    const nameCol = memNameKey();
    if (entry.kind === "preset") return (members.records || []).filter((rec) => entry.preset.match(rec, nameCol));
    return crVisibleFrom(entry.list.state);
  }
  function alAttSortVal(_key, name) {
    const a = attendanceInfoRange(name, alFullRange());
    return a.tier === "unknown" ? -1 : a.pct;
  }
  function alRows(entry, opts) {
    opts = opts || {};
    const nameCol = memNameKey();
    const done = alDoneSet(entry.id);
    let rows = alMatchingRecords(entry);
    if (!opts.includeDone) rows = rows.filter((r) => !done.has(nameKey(r[nameCol])));
    if (opts.q) { const q = opts.q.toLowerCase(); rows = rows.filter((r) => String(r[nameCol] || "").toLowerCase().includes(q)); }
    if (opts.sort) rows = sortRows(rows, opts.sort, nameCol, alAttSortVal);
    return rows;
  }
  function alCount(entry) { return alRows(entry).length; }

  // Live red counts on every list chip and the tab. Safe to call before the
  // attendance data loads (attendance-based presets simply hide until then).
  function refreshActionBadges() {
    let total = 0;
    for (const e of alAllLists()) total += alCount(e);
    const tb = $("actions-tab-badge");
    if (tb) { tb.textContent = String(total); tb.hidden = total === 0; }
    if (activeTab === "actions") renderAlSidebar();
  }
  ...
  function renderAlSidebar() {
    ...
    if (!ul) return;
    ul.textContent = "";
    const lists = alAllLists();
    const active = alActiveEntry();
    for (const e of lists) {
      const li = el("li", "cr-list-item al-item" + (active && e.id === active.id ? " is-active" : ""));
      const open = el("button", "cr-list-name al-open", null);
      open.type = "button";
      open.appendChild(el("span", "al-open-name", e.name));
      const n = alCount(e);
      open.appendChild(el("span", "al-badge" + (n === 0 ? " is-zero" : ""), String(n)));
      open.onclick = () => { alActiveId = e.id; alEditing = false; saveSort("actionsActive", alActiveId); renderActions(); };
      li.appendChild(open);
      if (e.kind === "custom") {
        const del = el("button", "cr-list-del", "×");
        del.type = "button"; del.title = "Delete list";
        del.onclick = (ev) => { ev.stopPropagation(); alDeleteList(e.id); };
        li.appendChild(del);
      }
      ul.appendChild(li);
    }
  ...
  function alFilterCtx() {
    const entry = alActiveEntry();
    return {
      state: entry.list.state,
      container: $("al-filters"),
      rebuild: () => { renderCrGroups(alFilterCtx()); alSaveLists(); renderAlTable(); refreshActionBadges(); },
      refresh: () => { alSaveLists(); renderAlTable(); refreshActionBadges(); },
    };
  }
  ...
  function renderActions() {
    ...
    renderAlSidebar();
    const entry = alActiveEntry();
    const nameInput = $("al-name");
    const editBtn = $("al-edit"), dupBtn = $("al-duplicate"), panel = $("al-edit-panel");
    if (!entry) {
      $("al-table").innerHTML = ""; $("al-count").textContent = ""; $("al-summary").textContent = "";
      const d0 = $("al-desc"); if (d0) { d0.hidden = true; d0.value = ""; }
      return;
    }
    ...
    if (entry.kind === "custom") {
      ...
      editBtn.style.display = ""; dupBtn.style.display = "";
      editBtn.classList.toggle("is-active", alEditing);
      panel.hidden = !alEditing;
      $("al-match").value = entry.list.state.groupsMatch;
      $("al-summary").textContent = alSummaryText(entry);
      if (alEditing) renderCrGroups(alFilterCtx());
      desc.hidden = false;
      desc.value = entry.list.state.description || "";
      desc.classList.toggle("is-empty", !desc.value.trim());
    } else {
      nameInput.value = entry.name;
      nameInput.readOnly = true;
      editBtn.style.display = "none"; dupBtn.style.display = "none";
      panel.hidden = true;
      $("al-summary").textContent = entry.preset.desc;
      // A preset's wording is fixed, so there is nothing to describe.
      desc.hidden = true;
      desc.value = "";
    }
    applySummaryVis("actions");
    renderColumnPicker(alColCtx());
    renderAlTable();
    refreshActionBadges();
  }

  // ------------------------------------------------ summary show/hide
  // Whether a report shows its summary line is remembered per report, in the
  // server-side prefs, and carries into the printed page and the spreadsheet
  // — hide it on screen and it is gone from the export too.
  // The flag lives with the report or list it belongs to, so it follows a
  // draft when it is named and travels with a duplicate. Action-list presets
  // are the exception — they have no stored state, so theirs go in a prefs
  // map keyed by preset id.
  // A preset action list has no saved definition of its own, so every
  // per-preset display choice — chosen columns, summary visibility — lives in
  // one prefs map keyed by preset id. Custom lists keep theirs on the list.
  const alPresetPrefs = (id) => (_loadPref("presetPrefs", {}) || {})[id] || {};
  function setAlPresetPrefs(id, patch) {
    const all = _loadPref("presetPrefs", {}) || {};
    all[id] = { ...(all[id] || {}), ...patch };
    _savePref("presetPrefs", all);
  }
  const SUMMARY_SCOPES = {
    custom: {
      sum: "cr-summary", btn: "cr-summary-btn",
      get: () => !!crState.summaryHidden,
      set: (v) => { crState.summaryHidden = v; crPersistActive(); },
    },
    actions: {
      sum: "al-summary", btn: "al-summary-btn",
      get: () => {
        const e = alActiveEntry();
        if (!e) return false;
        return e.kind === "custom" ? !!e.list.state.summaryHidden : !!alPresetPrefs(e.id).summaryHidden;
      },
      set: (v) => {
        const e = alActiveEntry();
        if (!e) return;
        if (e.kind === "custom") { e.list.state.summaryHidden = v; alSaveLists(); }
        else setAlPresetPrefs(e.id, { summaryHidden: v });
      },
    },
  };
  const isSummaryHidden = (scope) => SUMMARY_SCOPES[scope].get();
  function applySummaryVis(scope) {
    const s = SUMMARY_SCOPES[scope];
    const hidden = isSummaryHidden(scope);
    const sum = $(s.sum), btn = $(s.btn);
    if (sum) sum.classList.toggle("is-hidden", hidden);
    if (btn) {
      const label = hidden ? "Show summary" : "Hide summary";
  ...
  function toggleSummary(scope) {
    SUMMARY_SCOPES[scope].set(!isSummaryHidden(scope));
    applySummaryVis(scope);
  }
  // The subtitle a report carries into print/export: nothing when the user
  // has hidden the summary.
  const crSubtitleText = () => (isSummaryHidden("custom") ? "" : crSummaryText());
  const alSubtitleText = () => {
    if (isSummaryHidden("actions")) return "";
    const el = $("al-summary");
    return (el && el.textContent) || "";
  };

  function alSummaryText(entry) {
    const active = [];
    for (const g of entry.list.state.groups) for (const f of g.filters) if (f.field && f.op) active.push(crHumanFilter(f));
    if (!active.length) return "No conditions yet — this list matches everyone. Edit to narrow it down.";
    return "Members where " + active.join(entry.list.state.groupsMatch === "any" ? " — or — " : " and ") + ".";
  }

  function alDoneTh() {
    const th = el("th", "rm-th al-done-th", "Done");
    return th;
  }
  ...
  function renderAlTable() {
    if (!members) return;
    const entry = alActiveEntry();
    const table = $("al-table");
    if (!entry) { table.innerHTML = ""; return; }
    const nameCol = memNameKey();
    const q = ($("alq").value || "").trim();
    const done = alDoneSet(entry.id);
    const rows = alRows(entry, { q, sort: alSort, includeDone: alShowDone });
    const outstanding = alCount(entry);

    const byKey = new Map((members.columns || []).map((c) => [c.key, c]));
    const baseCols = alColumnsOf(entry).map((k) => byKey.get(k)).filter(Boolean);
    ATT_COL.label = "Attendance";
    const displayCols = withAttendanceCol(baseCols, nameCol, false);

    const thead = el("thead");
    const htr = el("tr");
    htr.appendChild(numTh());
    htr.appendChild(alDoneTh());
    const alCols = alColCtx();
    for (const c of displayCols) {
      htr.appendChild(columnTh(c, alCols));
    }
    thead.appendChild(htr);
    ...
    for (const [i, r] of rows.entries()) {
      ...
      const tr = el("tr", isDone ? "al-row-done" : null);
      tr.appendChild(numTd(i, "mem-td"));
      const dtd = el("td", "mem-td al-done-td");
      const cb = el("input"); cb.type = "checkbox"; cb.checked = isDone;
      cb.title = isDone ? "Checked off — click to restore" : "Mark as done";
      cb.onchange = () => { alSetDone(entry.id, key, cb.checked); renderAlTable(); refreshActionBadges(); };
      dtd.appendChild(cb);
      tr.appendChild(dtd);
      for (const c of displayCols) {
        if (c.att) {
          const td = el("td", "mem-td att-td");
          td.appendChild(attendanceBadgeRange(nm, alFullRange()));
          tr.appendChild(td);
          continue;
        }
        const v = r[c.key];
        if (c.key === nameCol && String(v || "").trim()) {
          const td = el("td", "mem-td"); td.appendChild(nameLink(String(v))); tr.appendChild(td);
        } else if (isCallingColumn(c) && !callingEligible(r)) {
          tr.appendChild(el("td", "mem-td mem-na", "(N/A)"));
        } else if (isTempleRecommendColumn(c)) {
    ...
    $("al-count").textContent = outstanding === 0
      ? (doneCount ? "All clear · " + doneCount + " checked off" : "All clear")
      : outstanding + (outstanding === 1 ? " person to action" : " people to action") +
        (doneCount ? " · " + doneCount + " checked off" : "");
    $("al-empty").hidden = rows.length > 0;
  }

  function alUniqueName(base) {
    const names = new Set(alLists.map((l) => l.name));
  ...
  function alNewList() {
    ...
    alLists.push({ id, name: alUniqueName("New list"), state: alNewState() });
    alSaveLists();
    alActiveId = id; alEditing = true; saveSort("actionsActive", alActiveId);
    renderActions();
    const ni = $("al-name"); if (ni) { ni.focus(); ni.select(); }
  }
  function alDuplicate() {
    const entry = alActiveEntry();
    if (!entry || entry.kind !== "custom") return;
    const id = "al-" + Date.now().toString(36);
    alLists.push({ id, name: alUniqueName(entry.name + " copy"), state: JSON.parse(JSON.stringify(entry.list.state)) });
    alSaveLists();
    alActiveId = id; alEditing = true; saveSort("actionsActive", alActiveId);
    renderActions();
  }
  function alDeleteList(id) {
    const list = alLists.find((l) => l.id === id);
    if (!list) return;
    if (!confirm("Delete the “" + list.name + "” list? Its check-offs are cleared too.")) return;
    alLists = alLists.filter((l) => l.id !== id);
    delete alDone[id];
    alSaveLists(); alSaveDone();
    if (alActiveId === id) { alActiveId = null; alEditing = false; }
    renderActions();
  }
  function alDownloadCsv() {
    ...
    const nameCol = memNameKey();
    const byKey = new Map((members.columns || []).map((c) => [c.key, c]));
    const wantCols = (entry.kind === "custom" && entry.list.state.columnsCustomized)
      ? entry.list.state.columns : crDefaultColumns();
    const cols = wantCols.map((k) => byKey.get(k)).filter(Boolean);
    const rows = alRows(entry, { sort: alSort });
    const esc = (s) => { s = String(s == null ? "" : s); return /[",\n]/.test(s) ? '"' + s.replace(/"/g, '""') + '"' : s; };
    const head = cols.map((c) => c.label).concat(["Attendance"]);
    const lines = [head.map(esc).join(",")];
    for (const r of rows) {
      const cells = cols.map((c) => r[c.key]);
      cells.push(attendanceInfoRange(r[nameCol], alFullRange()).text);
      lines.push(cells.map(esc).join(","));
    }
    const blob = new Blob([lines.join("\n")], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = entry.name.replace(/[^\w.-]+/g, "-") + ".csv";
    document.body.appendChild(a); a.click(); a.remove();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
  }

  // These controls are static markup, so they are wired the first time the
  // tab is opened and never again — deliberately NOT gated on the member
  // report having loaded. Gating them on data is what previously left the
  // whole panel dead (new list, search, show checked-off, the lot) whenever
  // the report had not arrived yet. Each handler no-ops safely on its own
  // until there is something to act on.
  function alWire() {
    if (alWired) return;
    alWired = true;
    $("al-new").onclick = alNewList;
    $("al-duplicate").onclick = alDuplicate;
    $("al-edit").onclick = () => { alEditing = !alEditing; renderActions(); };
    $("al-export").onclick = (e) => { e.preventDefault(); alDownloadCsv(); };
    $("al-name").addEventListener("input", () => {
      const entry = alActiveEntry();
      if (entry && entry.kind === "custom") { entry.list.name = $("al-name").value; alSaveLists(); renderAlSidebar(); }
    });
    $("al-match").addEventListener("change", () => {
      const entry = alActiveEntry();
      if (entry && entry.kind === "custom") { entry.list.state.groupsMatch = $("al-match").value; alFilterCtx().rebuild(); }
    });
    $("al-add-filter").addEventListener("click", () => {
      const entry = alActiveEntry();
      if (entry && entry.kind === "custom") { entry.list.state.groups.push({ match: "all", filters: [] }); alFilterCtx().rebuild(); }
    });
    $("alq").addEventListener("input", renderAlTable);
    $("al-showdone").addEventListener("change", () => { alShowDone = $("al-showdone").checked; renderAlTable(); });
  }

  function ensureActions() {
    alWire();
    // One retry once the report lands. Re-entering unconditionally would spin
    // forever when the report can't load at all (expired session, no network).
    if (!members) {
      if (!alAwaitingMembers) {
        alAwaitingMembers = true;
        ensureMembers().finally(() => {
          alAwaitingMembers = false;
          if (members && activeTab === "actions") ensureActions();
        });
      }
      return;
    }
  ...
  function crLoadSaved() { return _loadPref("customReports", {}); }
  ...
  function crLoad(name) {
    ...
    crApplyDesc();
    crSetEditing(false);
    renderCrSidebar(); renderCrGroups(); renderColumnPicker(crColCtx()); renderCrTable();
  }
  ...
  function crSaveCurrent() {
    ...
    // crCloneState is the one place that knows a report's shape — hand-building
    // the object here is how a newly added field silently fails to save.
    saved[name] = crCloneState();
    _savePref("customReports", saved);
    crActiveName = name;
    renderCrSidebar();
    crSetEditing(false);
  }
  ...
  function crCloneState() {
    return {
      ...
      summaryHidden: !!crState.summaryHidden,
  ...
  $("al-columns-btn").addEventListener("click", () => { $("al-columns").hidden = !$("al-columns").hidden; });
  $("qi-quarter").addEventListener("change", () => { qiQuarterId = $("qi-quarter").value; renderQuarterly(); });
  ...
  // Same for an action list's description, saved onto the list itself.
  let alDt;
  $("al-desc").addEventListener("input", () => {
    const d = $("al-desc");
    const entry = alActiveEntry();
    if (!entry || entry.kind !== "custom") return;
    entry.list.state.description = d.value;
    d.classList.toggle("is-empty", !d.value.trim());
    clearTimeout(alDt); alDt = setTimeout(alSaveLists, 200);
  });
  Object.keys(SUMMARY_SCOPES).forEach((scope) => {
    const b = $(SUMMARY_SCOPES[scope].btn);
    if (b) b.addEventListener("click", () => toggleSummary(scope));
  });
  $("cr-export").addEventListener("click", (e) => { e.preventDefault(); crDownloadCsv(); });
  $("qi-modal-close").addEventListener("click", qiCloseIndicator);
  $("qi-backdrop").addEventListener("click", qiCloseIndicator);
  $("qi-modal-csv").addEventListener("click", qiDownloadCsv);
  $("profile-close").addEventListener("click", closeProfile);
  $("profile-backdrop").addEventListener("click", closeProfile);
  document.addEventListener("keydown", (e) => {
    if (e.key !== "Escape") return;
    // Innermost first: a profile opened from the drill-down closes back to it.
    if (!$("profile-modal").hidden) closeProfile();
    else if (!$("qi-modal").hidden) qiCloseIndicator();
  });
  let rt;
  ...
  // The ward's Quarterly Report, computed from the two reports we hold: the
  // all-members export and the class & quorum attendance roll. Every indicator
  // says what it needs; when the export does not carry that field, or the
  // question is about something neither report answers, the row is marked "!"
  // rather than guessed at. Attendance-based rows count "attended at least
  // once in the quarter" against the selected quarter's weeks.
  const QI_FIELDS = {
    gender:    [/gender|\bsex\b/],
    priesthood:[/priesthood(?!.*office)/],
    office:    [/priesthood.*office/],
    marriage:  [/marriage|marital/],
    trStatus:  [/temple.*recommend.*status/],
    trExpiry:  [/temple.*recommend.*expir/],
    rm:        [/returned.*missionar/],
    baptism:   [/bapti/],
    callings:  [/calling/],
    household: [/head.*house|household/],
    endowed:   [/endow/],
    sealed:    [/seal/],
  };
  function qiCol(name) {
    const pats = QI_FIELDS[name] || [];
    const cols = (members && members.columns) || [];
    return cols.find((c) => pats.some((re) => re.test(`${c.key} ${c.label}`.toLowerCase()))) || null;
  }
  function qiVal(rec, name) {
    const c = qiCol(name);
    return c ? String(rec[c.key] == null ? "" : rec[c.key]).trim() : "";
  }
  const qiYes = (v) => /^(y|yes|true|1)\b/i.test(String(v || "").trim());

  // Age on a given YYYYMMDD, from the birth column the rest of the app uses.
  function qiAgeOn(rec, onNum) {
    const { birthKey } = callingCols();
    const b = birthKey ? crDate(rec[birthKey]) : null;
    if (b == null || onNum == null) return null;
    let age = Math.floor(onNum / 10000) - Math.floor(b / 10000);
    if ((onNum % 10000) < (b % 10000)) age -= 1;
    return age;
  }
  const qiInBand = (rec, on, lo, hi) => {
    const a = qiAgeOn(rec, on);
    return a != null && a >= lo && (hi == null || a <= hi);
  };
  const qiIsMale = (rec) => /^m/i.test(qiVal(rec, "gender"));
  const qiIsFemale = (rec) => /^f/i.test(qiVal(rec, "gender"));
  const qiIsMP = (rec) => /melchizedek/i.test(qiVal(rec, "priesthood") + " " + qiVal(rec, "office"));
  // A recommend counts as current when the status says so, or failing that
  // when the expiry date has not passed by the quarter end.
  function qiRecommendCurrent(rec, on) {
    const st = qiVal(rec, "trStatus");
    if (st) {
      if (/expired|cancel|none|lapsed|no\b/i.test(st)) return false;
      if (/active|current|valid|yes/i.test(st)) return true;
    }
    const c = qiCol("trExpiry");
    if (c) { const d = crDate(rec[c.key]); return d != null && d >= on; }
    return false;
  }
  // Aaronic/Melchizedek office ranking, for "office appropriate for age".
  const QI_OFFICE_RANK = [[/high\s*priest/i, 5], [/elder/i, 4], [/priest/i, 3], [/teacher/i, 2], [/deacon/i, 1]];
  function qiOfficeRank(rec) {
    const v = qiVal(rec, "office") + " " + qiVal(rec, "priesthood");
    for (const [re, rank] of QI_OFFICE_RANK) if (re.test(v)) return rank;
    return 0;
  }
  const qiExpectedRank = (age) => (age >= 18 ? 4 : age >= 16 ? 3 : age >= 14 ? 2 : age >= 12 ? 1 : 0);

  // Converts: baptized inside the window and old enough that it was not the
  // baptism of a child of record at eight.
  function qiConverts(recs, from, to) {
    const c = qiCol("baptism");
    if (!c) return null;
    return recs.filter((r) => {
      const d = crDate(r[c.key]);
  ...
  // Someone with no eligible Sunday in the quarter — baptized after the last
  // week pulled, or not on the roll at all — reads as 0/0, a blank rather than
  // a zero. They count as attending: an indicator should not be dragged down
  // by weeks nobody could have attended.
  const qiAttended = (rec, ctx) => countsAsAttending(rec[ctx.nameCol], ctx.range);
  // actual = how many of `pool` were seen in the quarter; potential = the pool.
  const qiAttendRatio = (pool, ctx) => ({
    actual: pool.filter((r) => qiAttended(r, ctx)),
    potential: pool,
  });
  const qiNeed = (...names) => names.every((n) => !!qiCol(n));

  const QI_ROWS = [
    { g: "Indicators of Conversion and Church Growth" },
    { n: 1, label: "Individuals who are sealed to a spouse in the temple",
      why: "The member export carries no sealing field.",
      calc: (c) => qiNeed("sealed", "marriage") && {
        actual: c.recs.filter((r) => qiYes(qiVal(r, "sealed")) || /seal/i.test(qiVal(r, "sealed"))),
        potential: c.recs.filter((r) => /married/i.test(qiVal(r, "marriage"))),
      } },
    { n: 2, label: "Endowed members with a current temple recommend",
      why: "The member export carries no endowment field.",
      calc: (c) => qiNeed("endowed") && (() => {
        const pool = c.recs.filter((r) => qiYes(qiVal(r, "endowed")));
        return { actual: pool.filter((r) => qiRecommendCurrent(r, c.end)), potential: pool };
      })() },
    { n: 3, label: "Males ages 18–25 serving or who have served a full-time mission",
      note: "Counts returned missionaries; the export does not flag those currently serving.",
      why: "The member export carries no returned-missionary field.",
      calc: (c) => qiNeed("rm", "gender") && (() => {
        const pool = c.recs.filter((r) => qiIsMale(r) && qiInBand(r, c.end, 18, 25));
        return { actual: pool.filter((r) => qiYes(qiVal(r, "rm"))), potential: pool };
      })() },
    { n: 4, label: "Adult males holding Melchizedek Priesthood",
      why: "The member export carries no priesthood field.",
      calc: (c) => qiNeed("priesthood", "gender") && (() => {
        const pool = c.recs.filter((r) => qiIsMale(r) && qiInBand(r, c.end, 18, null));
        return { actual: pool.filter(qiIsMP), potential: pool };
      })() },
    { n: 5, label: "Prospective elders ordained to the Melchizedek Priesthood in the last 12 months",
      why: "Neither report carries ordination dates." },
    { n: 6, label: "Young men with priesthood office appropriate for their age",
      why: "The member export carries no priesthood office field.",
      calc: (c) => qiNeed("office", "gender") && (() => {
        const pool = c.recs.filter((r) => qiIsMale(r) && qiInBand(r, c.end, 12, 17));
        return {
          actual: pool.filter((r) => qiOfficeRank(r) >= qiExpectedRank(qiAgeOn(r, c.end))),
          potential: pool,
        };
      })() },
    { n: 7, label: "Average sacrament meeting attendance",
      why: "The roll is class & quorum attendance; sacrament meeting attendance is a separate LCR count." },
    { n: 8, label: "Converts baptized and confirmed in the last 12 months", count: true,
      why: "The member export carries no baptism date.",
      calc: (c) => { const v = qiConverts(c.recs, c.yearAgo, c.end); return v && { actual: v }; } },
    { n: 9, label: "Adults and youth who submitted ancestor names for temple ordinances in the last 12 months",
      why: "Temple and FamilySearch submissions are in neither report." },

    { g: "Members / Families" },
    { n: 10, label: "Total members", count: true, calc: (c) => ({ actual: c.recs }) },
    { n: 11, label: "Total families", count: true,
      why: "The member export carries no household field.",
      calc: (c) => qiNeed("household") && (() => {
        const seen = new Set(), out = [];
        for (const r of c.recs) {
          const k = nameKey(qiVal(r, "household"));
          if (!k || seen.has(k)) continue;
          seen.add(k); out.push(r);
        }
        return { actual: out, unit: "households" };
      })() },
    { n: 12, label: "Ministering brother companionships interviewed",
      why: "Ministering assignments and interviews are in neither report." },
    { n: 13, label: "Ministering sister companionships interviewed",
      why: "Ministering assignments and interviews are in neither report." },

    { g: "Adults" },
    { n: 14, label: "Melchizedek Priesthood holders attending priesthood, Sunday School, or Primary meetings",
      why: "The member export carries no priesthood field.",
      // Same pool as #4, so the two rows agree even when the export records a
      // priesthood against somebody who cannot hold one.
      calc: (c) => qiNeed("priesthood", "gender") &&
        qiAttendRatio(c.recs.filter((r) => qiIsMale(r) && qiInBand(r, c.end, 18, null) && qiIsMP(r)), c) },
    { n: 15, label: "Prospective elders attending priesthood, Sunday School, or Primary meetings",
      note: "Prospective elder = adult male without the Melchizedek Priesthood.",
      why: "The member export carries no priesthood field.",
      calc: (c) => qiNeed("priesthood", "gender") &&
        qiAttendRatio(c.recs.filter((r) => qiIsMale(r) && qiInBand(r, c.end, 18, null) && !qiIsMP(r)), c) },
    { n: 16, label: "Women attending Relief Society, Sunday School, Young Women, or Primary meetings",
      why: "The member export carries no gender field.",
      calc: (c) => qiNeed("gender") &&
        qiAttendRatio(c.recs.filter((r) => qiIsFemale(r) && qiInBand(r, c.end, 18, null)), c) },
    { n: 17, label: "Young single adults attending priesthood, Relief Society, Sunday School, Young Women, or Primary meetings",
      note: "Young single adult = 18 to 30 and not married.",
      why: "The member export carries no marital status field.",
      calc: (c) => qiNeed("marriage") &&
        qiAttendRatio(c.recs.filter((r) => qiInBand(r, c.end, 18, 30) && !/married/i.test(qiVal(r, "marriage"))), c) },

    { g: "Youth" },
    { n: 18, label: "Young men attending priesthood and Sunday School meetings",
      why: "The member export carries no gender field.",
      calc: (c) => qiNeed("gender") &&
        qiAttendRatio(c.recs.filter((r) => qiIsMale(r) && qiInBand(r, c.end, 12, 17)), c) },
    { n: 19, label: "Young women attending Young Women and Sunday School meetings",
      why: "The member export carries no gender field.",
      calc: (c) => qiNeed("gender") &&
        qiAttendRatio(c.recs.filter((r) => qiIsFemale(r) && qiInBand(r, c.end, 12, 17)), c) },
    { n: 20, label: "Youth with a current proxy baptisms and confirmations temple recommend",
      note: "A youth recommend is the limited-use kind; the export does not name the type.",
      why: "The member export carries no temple recommend field.",
      calc: (c) => (qiNeed("trStatus") || qiNeed("trExpiry")) && (() => {
        const pool = c.recs.filter((r) => qiInBand(r, c.end, 12, 17));
        return { actual: pool.filter((r) => qiRecommendCurrent(r, c.end)), potential: pool };
      })() },

    { g: "Children" },
    { n: 21, label: "Children ages 0 through 2 years (as of 1 January)", count: true,
      why: "The member export carries no birth date.",
      calc: (c) => callingCols().birthKey
        ? { actual: c.recs.filter((r) => qiInBand(r, c.jan1, 0, 2)) }
        : false },
    { n: 22, label: "Children ages 3 and older attending Primary",
      why: "The member export carries no birth date.",
      calc: (c) => callingCols().birthKey &&
        qiAttendRatio(c.recs.filter((r) => qiInBand(r, c.end, 3, 11)), c) },

    { g: "Converts (past 12 months)" },
    { n: 23, label: "Converts attending at least one sacrament meeting last month",
      why: "The roll is class & quorum attendance; sacrament meeting attendance is a separate LCR count." },
    { n: 24, label: "Youth and adult converts assigned a Church responsibility or calling",
      why: "The member export carries no baptism date or callings field.",
      calc: (c) => qiNeed("callings") && (() => {
        const v = qiConverts(c.recs, c.yearAgo, c.end);
        if (!v) return false;
        const pool = v.filter((r) => qiInBand(r, c.end, 12, null));
        return { actual: pool.filter((r) => qiVal(r, "callings")), potential: pool };
      })() },
    { n: 25, label: "Youth and adult male converts ordained to appropriate priesthood office",
      why: "The member export carries no baptism date or priesthood office field.",
      calc: (c) => qiNeed("office", "gender") && (() => {
        const v = qiConverts(c.recs, c.yearAgo, c.end);
        if (!v) return false;
        const pool = v.filter((r) => qiIsMale(r) && qiInBand(r, c.end, 12, null));
        return { actual: pool.filter((r) => qiOfficeRank(r) >= qiExpectedRank(qiAgeOn(r, c.end))), potential: pool };
      })() },
    { n: 26, label: "Converts age 12 and older who have submitted ancestor names for temple ordinances in the last 12 months",
      why: "Temple and FamilySearch submissions are in neither report." },
  ];


  // ---------------------------------------------------- indicator drill-down
  // Clicking a row opens the people behind the number: everyone in the
  // potential pool, flagged by whether they count toward the actual. The gap
  // between the two is usually the point — the adult males without the
  // Melchizedek Priesthood, the youth who were not seen this quarter.
  // Remaining first: the gap is the reason to open a row — the adult males
  // without the Melchizedek Priesthood, the youth not seen this quarter.
  const QI_SCOPES = [["remaining", "Remaining"], ["actual", "Actual"], ["potential", "Potential"]];
  let qiOpenRow = null;
  let qiScope = "remaining";

  // The three lists behind an indicator. A count-only indicator has no pool to
  // fall short of, so it is all "actual" and the scopes collapse to one.
  function qiScopeLists(row, ctx) {
    const v = row.value;
    const inActual = new Set(v.actual.map((r) => nameKey(r[ctx.nameCol])));
    if (!v.potential) return { actual: v.actual, potential: null, remaining: null, inActual };
    return {
      actual: v.actual,
      potential: v.potential,
      remaining: v.potential.filter((r) => !inActual.has(nameKey(r[ctx.nameCol]))),
      inActual,
    };
  }
  const qiScopeRows = (lists, scope) =>
    (scope === "actual" ? lists.actual : scope === "potential" ? lists.potential : lists.remaining) || lists.actual;

  function qiOpenIndicator(row, ctx) {
    if (!row || !row.value) return;
    qiOpenRow = { row, ctx };
    qiScope = "remaining";
    qiRenderIndicator();
    $("qi-modal").hidden = false;
    document.body.classList.add("modal-open");
  }

  function qiRenderIndicator() {
    if (!qiOpenRow) return;
    const { row, ctx } = qiOpenRow;
    const v = row.value;
    const lists = qiScopeLists(row, ctx);
    const unit = v.unit || "people";

    $("qi-modal-title").textContent = `${row.n}. ${row.label}`;
    const pct = lists.potential && lists.potential.length
      ? Math.round((lists.actual.length / lists.potential.length) * 100) + "%" : null;
    $("qi-modal-meta").textContent =
      `${ctx.quarter.label} · as of ${qiHumanDate(ctx.end)} · ` +
      (lists.potential
        ? `${lists.actual.length} of ${lists.potential.length} ${unit}${pct ? " · " + pct : ""}`
        : `${lists.actual.length} ${unit}`);

    const body = $("qi-modal-body");
    body.textContent = "";

    if (lists.potential) {
      const nav = el("nav", "subtabs qi-scopes");
      for (const [v2, label] of QI_SCOPES) {
        const n = (qiScopeRows(lists, v2) || []).length;
        const b = el("button", "subtab" + (qiScope === v2 ? " is-active" : ""), `${label} ${n}`);
        b.type = "button";
        b.onclick = () => { qiScope = v2; qiRenderIndicator(); };
        nav.appendChild(b);
      }
      body.appendChild(nav);
    }

    const rows = (qiScopeRows(lists, qiScope) || []).slice()
      .sort((a, b) => String(a[ctx.nameCol] || "").localeCompare(String(b[ctx.nameCol] || "")));
    if (!rows.length) {
      body.appendChild(el("p", "modal-msg", qiScope === "remaining"
        ? "Nobody is outstanding — everyone in the pool already counts."
        : "Nobody falls into this group for the selected quarter."));
      return;
    }

    // Only the whole pool mixes the two, so only it needs the column.
    const withCounts = lists.potential && qiScope === "potential";
    const table = el("table", "rm-table mem-table qi-people");
    const thead = el("thead"), htr = el("tr");
    htr.appendChild(el("th", "rm-th num-th", "#"));
    htr.appendChild(el("th", "rm-th", "Name"));
    if (withCounts) htr.appendChild(el("th", "rm-th", "Counts"));
    htr.appendChild(el("th", "rm-th", "Attendance"));
    thead.appendChild(htr);
    const tbody = el("tbody");
    rows.forEach((r, i) => {
      const nm = String(r[ctx.nameCol] || "");
      const yes = lists.inActual.has(nameKey(nm));
      // Only meaningful where the two are mixed; in Remaining every row is
      // outstanding, so tinting them all would say nothing.
      const tr = el("tr", withCounts && !yes ? "qi-gap-row" : null);
      tr.appendChild(numTd(i, "mem-td"));
      const td = el("td", "mem-td");
      td.appendChild(nameLink(nm));
      tr.appendChild(td);
      if (withCounts) {
        const c = el("td", "mem-td");
        c.appendChild(el("span", "part part-" + (yes ? "yes" : "no"), yes ? "Yes" : "No"));
        tr.appendChild(c);
      }
      const att = el("td", "mem-td att-td");
      att.appendChild(attendanceBadgeRange(nm, ctx.range));
      tr.appendChild(att);
      tbody.appendChild(tr);
    });
  ...
  function qiCloseIndicator() {
    $("qi-modal").hidden = true;
    qiOpenRow = null;
    if ($("profile-modal").hidden) document.body.classList.remove("modal-open");
  }
  function qiDownloadCsv() {
    if (!qiOpenRow) return;
    const { row, ctx } = qiOpenRow;
    const lists = qiScopeLists(row, ctx);
    const rows = qiScopeRows(lists, qiScope) || [];
    const scopeLabel = (QI_SCOPES.find(([v]) => v === qiScope) || [])[1] || "Actual";
    const withCounts = lists.potential && qiScope === "potential";
    const esc = (x) => { const t = String(x == null ? "" : x); return /[",\n]/.test(t) ? '"' + t.replace(/"/g, '""') + '"' : t; };
    const head = ["Name"].concat(withCounts ? ["Counts"] : []).concat(["Attendance"]);
    const lines = [head.join(",")];
    for (const r of rows) {
      const nm = String(r[ctx.nameCol] || "");
      const cells = [nm];
      if (withCounts) cells.push(lists.inActual.has(nameKey(nm)) ? "Yes" : "No");
      cells.push(attendanceInfoRange(nm, ctx.range).text);
      lines.push(cells.map(esc).join(","));
    }
    const blob = new Blob([lines.join("\n")], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `${row.n} ${row.label} ${scopeLabel} ${ctx.quarter.label}`.replace(/[^\w .-]+/g, "").trim() + ".csv";
    document.body.appendChild(a); a.click(); a.remove();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
  }


  // Quarters the attendance roll covers, newest first, with the week indices
  // that fall inside each. Attendance-based indicators are scoped to these.
  function qiQuarters() {
    const weeks = ((data && data.weekOptions) || []).map((w) => crDate(w.date));
    const byId = new Map();
    weeks.forEach((n, i) => {
      if (n == null) return;
      const y = Math.floor(n / 10000);
      const q = Math.floor(((Math.floor(n / 100) % 100) - 1) / 3) + 1;
      const id = `${y}Q${q}`;
      if (!byId.has(id)) byId.set(id, { id, year: y, q, label: `${y} Q${q}`, start: i, end: i });
      else byId.get(id).end = i;
    });
    return [...byId.values()].reverse();
  }
  let qiQuarterId = null;
  function qiActiveQuarter() {
    const qs = qiQuarters();
    if (!qs.length) return null;
    return qs.find((x) => x.id === qiQuarterId) || qs[0];
  }
  // Quarter end as YYYYMMDD — the date the indicators are "as of".
  function qiQuarterEnd(q) {
    const endMonth = q.q * 3;
    const last = new Date(q.year, endMonth, 0).getDate();
    return q.year * 10000 + endMonth * 100 + last;
  }

  function qiContext() {
    const q = qiActiveQuarter();
    if (!q || !members) return null;
    const end = qiQuarterEnd(q);
    return {
      ...
      jan1: q.year * 10000 + 101,
      yearAgo: crShiftMonths(end, -12),
    };
  }

  function renderQuarterly() {
    const table = $("qi-table");
    if (!table) return;
    const sel = $("qi-quarter");
    const quarters = qiQuarters();
    sel.innerHTML = "";
    for (const q of quarters) {
      const o = el("option", null, q.label); o.value = q.id;
      if (qiActiveQuarter() && q.id === qiActiveQuarter().id) o.selected = true;
      sel.appendChild(o);
    }
    sel.disabled = quarters.length <= 1;

    const ctx = qiContext();
    if (!ctx) {
      table.innerHTML = "";
      $("qi-legend").textContent = members
        ? "No attendance weeks pulled yet, so the quarter cannot be set."
        : "Reading the reports…";
      return;
    }

    const rows = QI_ROWS.map((r) => {
      if (r.g) return r;
      let v = null;
      try { v = r.calc ? r.calc(ctx) : null; } catch (_) { v = null; }
      return { ...r, value: v || null };
    });
    const missing = rows.filter((r) => !r.g && !r.value).length;
    $("qi-legend").textContent =
      `Counts are as of ${qiHumanDate(ctx.end)}, from the all-members report and the class & quorum roll. ` +
      `Attendance means seen at least once in ${ctx.quarter.label}; a blank (0/0) — no eligible Sunday in the quarter — counts as attending. ` +
      `${missing} of ${rows.filter((r) => !r.g).length} indicators are marked ! — neither report answers them.`;

    const thead = el("thead");
    const htr = el("tr");
    htr.appendChild(el("th", "rm-th qi-num-th", "#"));
    htr.appendChild(el("th", "rm-th", "Indicator"));
    for (const h of ["Actual", "Potential", "%"]) htr.appendChild(el("th", "rm-th qi-fig-th", h));
    thead.appendChild(htr);

    const tbody = el("tbody");
    for (const r of rows) {
      if (r.g) {
        const tr = el("tr", "qi-group-row");
        const td = el("td", "qi-group", r.g);
        td.colSpan = 5;
        tr.appendChild(td);
        tbody.appendChild(tr);
        continue;
      }
      const tr = el("tr", r.value ? "qi-clickable" : "qi-row-na");
      if (r.value) {
        tr.tabIndex = 0;
        tr.setAttribute("role", "button");
        tr.title = "Show the people behind this number";
        tr.onclick = () => qiOpenIndicator(r, ctx);
        tr.onkeydown = (e) => {
          if (e.key === "Enter" || e.key === " ") { e.preventDefault(); qiOpenIndicator(r, ctx); }
        };
      }
      tr.appendChild(el("td", "mem-td qi-num", String(r.n)));

      const label = el("td", "mem-td qi-label");
      label.appendChild(el("div", "qi-name", r.label));
      const note = r.value ? r.note : r.why;
      if (note) label.appendChild(el("div", "qi-note", note));
      tr.appendChild(label);

      if (!r.value) {
        const bang = el("td", "mem-td qi-fig qi-bang", "!");
        bang.title = r.why || "Not derivable from the pulled reports.";
        tr.appendChild(bang);
        tr.appendChild(el("td", "mem-td qi-fig", "—"));
        tr.appendChild(el("td", "mem-td qi-fig", "—"));
      } else {
        const actual = r.value.actual.length;
        const potential = r.value.potential ? r.value.potential.length : null;
        tr.appendChild(el("td", "mem-td qi-fig qi-actual", String(actual)));
        tr.appendChild(el("td", "mem-td qi-fig", potential == null ? "—" : String(potential)));
        const pct = potential ? Math.round((actual / potential) * 100) : null;
        const td = el("td", "mem-td qi-fig");
        if (pct == null) td.textContent = "—";
        else td.appendChild(el("span", "part part-" + attendanceTier(pct), pct + "%"));
        tr.appendChild(td);
      }
      tbody.appendChild(tr);
    }
  ...
  function qiHumanDate(num) {
    if (num == null) return "";
    const y = Math.floor(num / 10000), m = Math.floor(num / 100) % 100, d = num % 100;
    return `${d} ${CR_MONTHS[m - 1]} ${y}`;
  }

  function ensureQuarterly() {
    if (!members) {
      ensureMembers().finally(() => { if (activeTab === "quarterly") renderQuarterly(); });
      renderQuarterly();
      return;
    }
    renderQuarterly();
  ...
  // fields and adds the "Kalayaan Ward Stewardship" header and confidential footer.
  function printReport() {
    const names = { attendance: "Attendance", returned: "Returned Missionaries", members: "Members", custom: "Custom Reports", actions: "Action Lists", quarterly: "Quarterly Indicators" };
    let title = names[activeTab] || "Report";
    if (activeTab === "custom" && crActiveName) title = crActiveName;
    if (activeTab === "actions") { const e = alActiveEntry(); if (e) title = e.name; }
    let when = "";
  ...
  ["att-pdf", "rm-pdf", "mem-pdf", "cr-pdf", "al-pdf", "qi-pdf"].forEach((id) => {
    const btn = $(id);
  ...
  // A normal <table> report (returned, members, custom, action lists).
  // opts.skip is a selector matched against the header cells; any column it
  // matches is left out of the export entirely (the action list's tick-box
  // column carries no data worth exporting).
  function serializeTable(sel, title, subtitle, opts) {
    const table = $(sel.replace(/^#/, ""));
    if (!table) return null;
    const heads = [...table.querySelectorAll("thead th")];
    const skip = (opts && opts.skip) || null;
    const drop = new Set(
      skip ? heads.reduce((acc, th, i) => (th.matches(skip) ? acc.concat(i) : acc), []) : []
    );
    const keep = (_, i) => !drop.has(i);
    const columns = heads.filter(keep).map((th) => th.textContent.trim());
    const rows = [...table.querySelectorAll("tbody tr")].map((tr) => ({
      cells: [...tr.children].filter(keep).map((td) => {
    ...
    return { title, subtitle: subtitle || "", ward: "Kalayaan Ward Stewardship", date: todayLong(),
  ...
  function serializeRoll() {
    ...
    return { title: "Attendance", subtitle: subtitle.trim(), ward: "Kalayaan Ward Stewardship", date: todayLong(),
             columns, rows, footer: REPORT_FOOTER };
  }
  ...
  function serializeCurrentReport() {
    ...
    if (activeTab === "custom") {
      ...
      return serializeTable("#cr-table", crActiveName || "Custom Report", desc || crSubtitleText());
    }
    if (activeTab === "quarterly") {
      const q = qiActiveQuarter();
      const payload = serializeTable("#qi-table", "Quarterly Indicators", q ? q.label : "");
      if (payload) payload.filename = `Quarterly Indicators ${q ? q.label : ""} ${payload.date}`.trim();
      return payload;
    }
    if (activeTab === "actions") {
      const entry = alActiveEntry();
      const desc = ($("al-desc") && $("al-desc").value.trim()) || "";
      const payload = serializeTable("#al-table", (entry && entry.name) || "Action List",
                                     desc || alSubtitleText(), { skip: ".al-done-th" });
      // A list's summary is a whole sentence ("Members where …"), which makes
      // a miserable filename. Keep it in the document header only.
      if (payload) payload.filename = `${payload.title} ${payload.date}`.trim();
      return payload;
    }
  ...
  async function downloadSheet(btn) {
    ...
    if (!payload.filename) payload.filename = `${payload.title}${payload.subtitle ? " - " + payload.subtitle : ""} ${payload.date}`.trim();
  ...
  ["att-sheet", "rm-sheet", "mem-sheet", "cr-sheet", "al-sheet", "qi-sheet"].forEach((id) => {
    const btn = $(id);
