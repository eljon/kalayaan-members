(() => {
  ...
  function ensureActions() {
    // Re-enter rather than just re-rendering: the wiring below is skipped on
    // this pass, and renderActions() alone would leave every control dead.
    if (!members) { ensureMembers().then(() => { if (activeTab === "actions") ensureActions(); }); return; }
    crInit();
    if (!alWired) {
      alWired = true;
